...
<style type="text/css">
	body select, body .formTable .select,
	body input[type="text"], .printForm select,
	.printForm .formTable .select, .printForm input[type="text"],
	body .formTable input[type="text"],
	body .formTable input[parser="dicttable"]{
		width: 200px;
	}
	...
	<c:choose>
		<c:when test="${fn:length(userList)==0}">
			<tr>
					...
					<td>
						<c:choose>
							<c:when test="${isMultiInstance}">
								<select name="extractUser" class="select">
				...
				<td>
					<select name="compType" class="select">
