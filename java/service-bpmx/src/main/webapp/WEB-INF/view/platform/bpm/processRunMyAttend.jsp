			...
			<div class="panel-search">
					<form id="searchForm" method="post" action="myAttend.ht">
							...
							<ul class="row">
								...
								<li><span class="label">结束时间:</span> <input  name="Q_beginendTime_DL"  class="inputText date" value="${param['Q_beginendTime_DL']}"/>
								<span class="label label_line">_ </span><input  name="Q_endendTime_DG" class="inputText date" value="${param['Q_endendTime_DG']}"/></li>
							</ul>
					</form>
			</div>
		</div>
			    ...
			    <display:table name="processRunList" id="processRunItem" requestURI="myAttend.ht" sort="external" cellpadding="1" cellspacing="1" class="table-grid">
					...
					<display:column title="管理" media="html" style="width:150px;" class="opsBtnb">
						...
						<c:if test="${processRunItem.recover==1}">
							<a href="javascript:;" onclick="recover(${processRunItem.runId})" class="link redo">追回</a>
						</c:if>
					</display:column>
				</display:table>
