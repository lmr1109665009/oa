		...
		*/
		function generateExpress(obj){
			...
			case 'number':
				...
				break;
			case 'date':
				flowVarValue = "com.suneee.core.util.TimeUtil.convertString(\""+flowVarValue+'\","yyyy-MM-dd")';
				switch(flowVarOperate){
				case "eq":
											...
											<tr>
												...
												<td colspan="3">
													<select id="action" onchange="change()" name="action">
														...
														<option value="3" <c:if test="${taskReminder.action==3}">selected="selected"</c:if>>执行退回操作</option>
														<option value="4" <c:if test="${taskReminder.action==4}">selected="selected"</c:if>>执行退回到发起人操作</option>
