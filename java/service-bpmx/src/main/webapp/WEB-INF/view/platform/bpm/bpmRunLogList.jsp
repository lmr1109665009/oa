...
<body>
			...
			<div class="hide-panel">
				<div class="panel-top">
								...
								<form id="searchForm" method="post" action="list.ht">
										<ul class="row">
											...
											<select name="Q_operatortype_S" class="select">
												...
												<option value="8" <c:if test="${param['Q_operatortype_S'] == 8}">selected</c:if>>退回</option>
												<option value="9" <c:if test="${param['Q_operatortype_S'] ==9}">selected</c:if>>退回到发起人</option>
											...
											<li>
											<span class="label">操作时间:</span> <input  name="Q_begincreatetime_DL"  class="inputText date"  value="${param['Q_begincreatetime_DL']}"/>
					...
					<c:if test="${runId!=0}"><div class="panel-search"></div></c:if>
				...
				<div class="panel-body">
					...
					<display:table name="bpmRunLogList" id="bpmRunLogItem" requestURI="list.ht" sort="external" cellpadding="1" cellspacing="1" class="table-grid">
						...
						<display:column  title="流程标题"  style="width:270px;" sortable="true" sortName="processSubject">
							<c:choose>
						...
						<display:column property="username" title="用户名称" style="width:100px;" sortable="true" sortName="username"></display:column>
						<display:column  title="操作时间" sortable="true" style="width:180px;" sortName="createtime">
							<fmt:formatDate value="${bpmRunLogItem.createtime}" pattern="yyyy-MM-dd HH:mm:ss"/>
						</display:column>
						<display:column title="操作类型" sortable="true" style="width:160px;" sortName="operatortype">
							<c:choose>
								...
								<c:when test="${bpmRunLogItem.operatortype eq 8}"><span class="red">退回</span></c:when>
								<c:when test="${bpmRunLogItem.operatortype eq 9}"><span class="red">退回到发起人</span></c:when>
						...
						<display:column property="memo" title="备注" style="width:320px;" sortable="true" sortName="memo" maxLength="30"></display:column>
						<display:column title="管理" media="html" style="width:160px;" class="opsBtnb">
							<a href="get.ht?id=${bpmRunLogItem.id}" class="link detail">详情</a>  |
