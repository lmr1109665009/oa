	...
	<div class="hide-panel">
		...
		<div class="panel-body">
			    ...
			    <display:table name="processRunList" id="processRunItem" requestURI="alreadyCompletedMattersList.ht" sort="external" cellpadding="1" cellspacing="1" class="table-grid">
					...
					<!-- 是否有全局流水号 -->
					<c:if test="${hasGlobalFlowNo }">
						<display:column property="globalFlowNo" title="工单号" sortable="true"  sortName="globalFlowNo" style="text-align:left;width:12%"></display:column>
					</c:if>
					<display:column  titleKey="请求标题" sortable="true" sortName="subject" style="text-align:left;width:25%">
					...
					<display:column  titleKey="创建时间" sortable="true" sortName="createtime" style="text-align:left;width:15%">
						<fmt:formatDate value="${processRunItem.createtime}" pattern="yyyy-MM-dd HH:mm:ss"/>
					</display:column>
					<display:column titleKey="持续时间" style="text-align:left;width:12%" >					
						<c:choose>
