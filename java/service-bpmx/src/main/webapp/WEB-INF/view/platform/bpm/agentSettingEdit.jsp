		...
		*/
		...
		function showResponse(responseText) {
			...
			if (obj.isSuccess()) {
				...
				$.ligerDialog.err("提示信息","代理设定保存失败!",obj.getMessage());
			}
		}
			...
			<table class="table-detail" cellpadding="0" cellspacing="0" border="0" type="main">
				<c:if test="${agentType==1}">  <!-- 管理入口时 -->
					<tr>
						...
						<td colspan="3">
							...
							<a href="javascript:;" class="current" onclick="selectAuth();"><span>选择授权人</span></a>
						</td>
					</tr>				
				</c:if>
				...
				<tr id="trAgent"  <c:if test="${agentSetting.authtype==2}"> style="display: none"</c:if> >
					...
					<td colspan="3">
						...
						<a href="javascript:;" class="current" onclick="selectAgent();"><span>选择代理人</span></a>
					</td>
				</tr>
				<tr>
					<th>是否有效:</th>
				...
				<tr id="trDefinition" <c:if test="${ agentSetting.authtype !=2 }">style="display:none;" </c:if> >
					...
					<td colspan="3">
						...
						<a href="javascript:;" class="current" onclick="selectFlow();"><span>选择流程</span></a>
						<a href="javascript:;" class="current" id="linkAgentCondition" onclick="setCondition();"><span>设置代理条件</span></a>
					</td>
				</tr>
			</table>
	</div>
