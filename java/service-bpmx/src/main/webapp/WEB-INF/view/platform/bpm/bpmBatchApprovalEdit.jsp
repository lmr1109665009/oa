	...
	<script type="text/javascript">
		...
		function showResponse(responseText) {
			...
			if (obj.isSuccess()) {
				...
				$.ligerDialog.err("提示信息","流程批量审批定义保存失败!",obj.getMessage());
			}
		}
		...
		*/
		...
		function setFields(){
			...
			var url=__ctx + "/platform/bpm/bpmBatchApproval/fieldDialog.ht?defKey="+defKey+"&nodeId="+nodeId+"&fieldJson="+encodeURI(encodeURI(fieldJson));
			//url=url.getNewUrl();
			DialogUtil.open({
				height:500,
				width: 650,
         ...
         */
		function cancelFlow() {
			$("#bpmBatchApprovalForm")[0].reset();//整个表单会重置
            $("#defKey").val("");
            $("#nodeId").val("");
        }
