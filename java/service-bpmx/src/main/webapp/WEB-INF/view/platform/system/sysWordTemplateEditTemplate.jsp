...
<script type="text/javascript">
	...
	function showResponse(responseText){
		...
		if(obj.isSuccess()){
			...
			$.ligerDialog.err("提示信息","WORD表单模板保存失败!",obj.getMessage());
		}
	...
	function initZtree(){
		...
		for(var i=0,c;c=json.subtables[i++];){
			...
			treeData.push(c);
			//添加序号
			var rn={pId:c.id,fieldName:"rn",fieldDesc:"序号", name:"序号",icon:iconFolder +"number" + '.png'};
			treeData.push(rn);
...
<body style="overflow:hidden">
	...
	<div  class="panel-body">
		<form id="frmWorkFlow" method="post" action="save.ht">
			...
			<textarea name="sql" hidden="true">${sysWordTemplate.sql}</textarea>
			...
			<div id="tab" class="panel-nav">
				<div id="frmDefLayout">
					...
					<div id="wordEditor" position="center"  style="height:100%;">
						<input type="hidden" name="fileId" controltype="office"  value="${sysWordTemplate.fileId}"  right="w" menuRight="{}" fileType="wordTemplate"/>
					</div>
				</div>
			</div>
		</form>
