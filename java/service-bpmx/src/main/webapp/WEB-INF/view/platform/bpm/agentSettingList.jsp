	...
	<div class="hide-panel">
		<div class="panel-top">
			...
			<div class="panel-toolbar">
				<div class="toolBar">
					...
					<div class="l-bar-separator"></div>
					<div class="group"><f:a alias="EditAgent" css="link update update-hide" id="btnUpd" action="edit.ht" showNoRight="false"  ><span></span>修改</f:a></div>
					<div class="l-bar-separator"></div>
					<div class="group"><f:a alias="DelAgent" css="link del"  action="del.ht" showNoRight="false"  ><span></span>批量删除</f:a></div>
			...
			<div class="panel-search">
				<form id="searchForm" method="post" action="list.ht">
					<ul class="row">
					<li>
						<span class="label">开始时间:</span><input  name="Q_beginstartdate_DL"  class="inputText datePicker"  datetype="1" value="${param['Q_beginstartdate_DL']}" />
						<span class="label label_line">_ </span><input  name="Q_endstartdate_DG"  class="inputText datePicker"  datetype="2" value="${param['Q_endstartdate_DG']}" />
					</li>
					<li>
					...
					<li>
						<span class="label">结束时间:</span><input  name="Q_beginenddate_DL"  class="inputText datePicker"  datetype="1" value="${param['Q_beginenddate_DL']}" />
						<span class="label label_line">_ </span><input  name="Q_endenddate_DG"  class="inputText datePicker"  datetype="2" value="${param['Q_endenddate_DG']}"/>
					</li>
		...
		<div class="panel-body">
		    ...
		    <display:table name="agentSettingList" id="agentSettingItem" requestURI="list.ht" sort="external" cellpadding="1" cellspacing="1"  class="table-grid">
				...
				<display:column title="流程名称" sortable="true" sortName="authtype">
					<c:choose>
						<c:when test="${agentSettingItem.authtype==0}">全部流程</c:when>
						<c:when test="${agentSettingItem.authtype==1}">部分流程</c:when>
						<c:when test="${agentSettingItem.authtype==2}">条件部分</c:when>
					</c:choose>
				</display:column>
				...
				<display:column titleKey="管理" media="html" class="opsBtnb">				
					<f:a alias="EditAgent" css="link edit"  href="edit.ht?id=${agentSettingItem.id}"  ><span></span>编辑</f:a> |
					<f:a alias="DelAgent" css="link del"  href="del.ht?id=${agentSettingItem.id}"  ><span></span>删除</f:a>
				<%--
					<a href="get.ht?id=${agentSettingItem.id}" class="link detail">详情</a>  | 
					 --%>
				</display:column>
			</display:table>
