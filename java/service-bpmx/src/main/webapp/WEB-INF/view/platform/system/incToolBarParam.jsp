...
%>
	<input title='paramValue' <%if(param.getDataType().equals("Date")){ %>class="date" <%}else{%>class="inputText "<%}%>type="text" name="<%=paramId%>" value="<%=paramValue%>" style="width: 200px;">
<%
}
...
<%
 ...
 %>
     <div id="<%=paramId%>"> 
		<input value="<%=paramValue%>" nodeKey="<%=sourceKey%>" id="<%=paramId%>" class="dicCombo" catKey="DIC" valueField="dictType" isNodeKey="true" name="<%=paramId%>" height="150" width="150"/>
	</div>
<%} %>
	</td>
