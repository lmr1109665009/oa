  ...
  <c:if test="${buttonFlag}">  
	...
	<div class="hide-panel">
		<div class="panel-top">
			...
			<div class="panel-toolbar panel-toolbar-bo">
						...
						<c:choose>
							<c:when test="${item.isstartform==1 }">
								<c:choose>
									...
									<c:when test="${item.operatortype==6 }">保存</c:when>
							...
							<c:otherwise>
								<c:choose>
									<c:when test="${item.operatortype==1 }">批准</c:when>
									<c:when test="${item.operatortype==2 }">反对</c:when>
									<c:when test="${item.operatortype==3 }">弃权</c:when>
									<c:when test="${item.operatortype==4 }">退回</c:when>
									<c:when test="${item.operatortype==5 }">退回到发起人</c:when>
									...
									<c:when test="${item.operatortype==18 }">转发</c:when>
								</c:choose>
							</c:otherwise>
						</c:choose>
