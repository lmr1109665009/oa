...
<body ng-controller="sysQueryMetaFieldCtrl">
		...
		<form>
			<div class="panel-detail">
				...
				<fieldset style="margin-top: 10px;padding: 0px 15px 20px 10px ;">
					<legend class="legend" onclick="$(this).next().toggle()" title="单击展开/收缩"> 表格格式设置 
					</legend>
					<div class="tipbox">
											<a href="javascript:;" class="tipinfo">
												<span>这个格式javascript代码。代码返回列格式化后的结果，这些代码作为function sex_Formater(cellvalue, options, rowObject){},函数体的一部分。[cellvalue]:为列代表的值，[rowObject]：表示为一行数据，可以通过 rowObject.sex类似的代码获取改行其他列的数据。</span>
											</a>
					</div>
					
					<br>
					<textarea rows="8" ng-model="sysQueryField.formater"
						 class="w100 h100 margin-none" style="resize: none;margin-top: 50px;"></textarea>
	...
	<div class="panel" ng-if="type=='sqlSetting'" ng-controller="sysQueryMetaSqlSettingCtrl">
		<dialog-buttons></dialog-buttons>
		<form >
			<div class="panel-detail">
				<table class="table-detail" cellpadding="0" cellspacing="0"
