				...
				<div class="panel-table">
	   				<table id="dicTable" class="table-grid table-list" id="0" cellpadding="1" cellspacing="1">
			    		...
			    		<c:forEach items="${sysOrgList}" var="sysOrgItem" varStatus="status">
			    			...
			    			<td nowrap="nowrap">${f:returnSpace(sysOrgItem.path)}<a name="tree_a" class="tree-list-minus" onclick="treeClick(this)"><span class="tree-list-span">${sysOrgItem.orgName }</span></a>
			    			</td>
			    			...
			    			<c:choose>
			   				...
			   				<td style="width:150px" class="opsBtnb">
			    			
							 <a href="edit.ht?orgId=${sysOrgItem.orgId}&demId=${sysOrgItem.demId}&flag=LE&path=${path}" class="link edit">编辑</a> |
							 <a href="get.ht?orgId=${sysOrgItem.orgId}&flag=1&path=${path}" class="link detail">详情</a> | 
