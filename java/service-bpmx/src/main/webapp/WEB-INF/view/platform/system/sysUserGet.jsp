...
<body>
		...
		<div class="panel-toolbar" id="pToolbar">
			...
			<c:if test="${canReturn==0 && openType!='detail'}">
				<c:set var="returnUrlStr" value="${returnUrl}"/>
					<c:if test="${fn:contains(returnUrlStr, 'type')}">
						<div class="group">
							<a class="link back" href="${returnUrl}#/task/toStart" ><span></span>返回</a>
						</div>
					</c:if>
					<c:if test="${fn:contains(returnUrl, 'link')}">
						<div class="group">
							<a class="link back" href="${returnUrl}#/task/processRunInfo" ><span></span>返回</a>
						</div>
					</c:if>
			</c:if>
		...
		<div title="基本信息" tabid="userdetail"
			...
			<div class="panel-detail">
				<table class="table-detail" cellpadding="0" cellspacing="0"
					border="0">
					<tr>
						<td rowspan="17" align="center" width="26%">
							<div class="person_pic_div">
								<p><img src="${ctx}/${pictureLoad}" style="height: 140px; width:140px;border-radius: 50%" alt="个人相片"  /></p>
							</div>
						</td>
					...
					<tr>
						<th style="height: 48px !important">字号:</th>
						<td>${sysUser.aliasName}</td>
					</tr>
					<tr>
					...
					<tr>
						<th style="height: 48px !important">工号:</th>
						<td>${sysUser.staffNo}</td>
					</tr>
					
					<tr>
						<th style="height: 48px !important">身份证号:</th>
						<td>${sysUser.identification}</td>
					</tr>
					
					<tr>
						<th style="height: 48px !important">考勤编号:</th>
						<td>${sysUser.attendNo}</td>
					</tr>
					<tr>
						<th>工作日期：</th>
						<td><fmt:formatDate value='${sysUser.workDate}' pattern='yyyy-MM-dd'/></td>
					</tr>
				</table>
			</div>
		</div>
