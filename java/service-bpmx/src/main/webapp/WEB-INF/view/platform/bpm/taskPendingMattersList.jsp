		...
		<div class="panel-top">
			...
			<div class="panel-toolbar">
				<div class="toolBar">
					...
					<div class="group"><f:a alias="batchApproval" css="link icon-wodejihua" id="btnApprove" href="javascript:;" showNoRight="false" onclick="summaryBatch()"><span></span>批量审批</f:a></div>
				...
				<form id="searchForm" method="post" action="pendingMattersList.ht?porIndex=${porIndex}&tabIndex=${tabIndex}">
					<ul class="row">
						...
						<li>
							...
							<input type="button" value="选择..."  class="buttonSearch"  onclick="selectUser('creatorId','creator');">
						</li>
					</ul>
					<ul class="row">
						<li>
						...
						<li>
							...
							<select name="Q_status_N"   class="selectSearch">
								...
								<option value="6" <c:if test="${param['Q_status_N'] == '6'}">selected</c:if>>已退回</option>
							</select>
						</li>
		...
		<div class="panel-body">
			    ...
			    <display:table name="taskList" id="taskItem" requestURI="pendingMattersList.ht" sort="external" cellpadding="1" cellspacing="1"  class="table-grid">
					...
					<display:column title="事项名称"  sortable="true" sortName="subject"  style="text-align:left;" >
							${f:subString(taskItem.subject)}<%-- <c:choose> 
					...
					<c:choose> 
									<c:when test="${taskItem.description eq '36'}">
										<a name="subject"  href="#${taskItem.id}" onclick="javascript:restartTask(${taskItem.id})" style="color:${warningSet[taskItem.priority].color}" title='${taskItem.subject}'  <c:if test="${taskItem.hasRead == 0}"> class='message close-message'</c:if><c:if test="${taskItem.hasRead != 0}"> class='message open-message'</c:if> >审批</a>
									</c:when>
								<c:otherwise>
										<a name="subject"  href="#${taskItem.id}" onclick="javascript:executeTask(${taskItem.id})" style="color:${warningSet[taskItem.priority].color}" title='${taskItem.subject}' <c:if test="${taskItem.hasRead == 0}"> class='message close-message'</c:if><c:if test="${taskItem.hasRead != 0}"> class='message open-message'</c:if> >审批</a>
								</c:otherwise>
							</c:choose>
					</display:column>
				</div>
