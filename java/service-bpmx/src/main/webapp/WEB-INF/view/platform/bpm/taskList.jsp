...
<script type="text/javascript">
	...
	function init(){
		var layout=$("#taskLayout").ligerLayout({rightWidth:300,height: $(window).height()-50,isRightCollapse:true});
		$("tr.odd,tr.even").each(function(){
			...
			$(".l-layout-header-toggle", this.right).click(function ()
		            {
				    resizeAchangeScrollHeight();
		            });
	...
	function changeHeight(){
		...
		//tableBody.height($(window).height()-$(".panel-top").height()-70);
...
<style type="text/css"> 
    ...
    #taskLayout .l-layout-right {overflow-y: auto;}
 </style>
</head>
<body>
			...
			<div  position="center" >
					...
					<div class="panel-body" >
					    ...
					    <display:table name="taskList" id="taskItem" requestURI="list.ht" sort="external" cellpadding="1" cellspacing="1"   class="table-grid">
							...
							<!-- 是否有全局流水号 -->
							<c:if test="${hasGlobalFlowNo }">
								<display:column property="globalFlowNo" title="工单号" sortable="true"  sortName="globalFlowNo" style="text-align:center;width:16%"></display:column>
							</c:if>
							<display:column media="html" title="事项名称" sortable="true" sortName="subject" style="text-align:center;width:25%" >
								<a href="#${taskItem.id}" onclick="javascript:executeTask(${taskItem.id},'${taskItem.name}')" >${taskItem.subject}</a>
							</display:column>
							<display:column property="name" title="任务名称" sortable="true"  sortName="name_" style="text-align:center;width:16%"></display:column>
							
							<display:column title="执行人" sortable="true" sortName="assignee_" style="text-align:center;width:13%">
								<f:userName userId="${taskItem.assignee}" taskId="${taskItem.id}"/>
							</display:column>
							<display:column title="状态" sortable="true" sortName="delegation_" style="text-align:center;width:13%">
								<c:choose>
									<c:when test="${empty taskItem.delegationState}">待执行</c:when>
									<c:otherwise>${taskItem.delegationState}</c:otherwise>
								</c:choose>
							</display:column>
							<display:column title="任务创建时间" sortable="true" sortName="create_time_" style="text-align:center;width:20%">
