								...
								<td><!--  nowrap="nowrap" -->
									...
									<c:choose>
										...
										<c:otherwise>
											<c:forEach items="${btnList}" var="btn" varStatus="status" >
												<c:choose>
													<c:when test="${btn.eventTag==0 }">
														<span >${btn.btnname}</span>
													</c:when>
													<c:when test="${btn.eventTag==1}">
														<span style="color:red;font-weight: bold;" title="前置事件">${btn.btnname}</span>
													</c:when>
													<c:when test="${btn.eventTag==2 }">
														<span style="color:blue;font-weight: bold;" title="后置事件">${btn.btnname}</span>
													</c:when>
													<c:when test="${btn.eventTag==3 }">
														<span style="color:brown;font-weight: bold;" title="前后置事件">${btn.btnname}</span>
													</c:when>
												</c:choose>
