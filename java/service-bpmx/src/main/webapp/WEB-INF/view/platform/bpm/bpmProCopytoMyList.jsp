	...
	function mark(t){
		...
		if(t){
			...
			$("input.pk").each(function(){
				...
				if(state&&me.closest('tr').find('td').eq(3).text().indexOf('未读')>0)
					idArr.push(me.val());
			});
			if(idArr.length==0){
			...
			<div class="panel-search">
				<form id="searchForm" method="post" action="myList.ht?porIndex=${porIndex}&tabIndex=${tabIndex}">
					<ul class="row">
						...
						<span class="label label_line">_ </span><input  name="Q_endcreatetime_DG" class="inputText datePicker" datetype="2" value="${param['Q_endcreatetime_DG']}"/></li>

						<li>
						<span class="label">接收时间:</span><input  name="Q_beginccTime_DL"  class="inputText datePicker" datetype="1"  value="${param['Q_beginccTime_DL']}" />
						<span class="label label_line">_ </span><input  name="Q_endccTime_DG" class="inputText datePicker" datetype="2" value="${param['Q_endccTime_DG']}"/></li>
		...
		<div class="panel-body">
		    ...
		    <display:table name="bpmProCopytoList" id="bpmProCopytoItem" requestURI="myList.ht" sort="external" cellpadding="1" cellspacing="1" class="table-grid">
				...
				<display:column title="请求标题"  sortable="true" sortName="subject">
					${f:subString(bpmProCopytoItem.subject)}
				</display:column>
				...
				<a onclick="showDetail(this)" href="#${bpmProCopytoItem.runId}" action="${ctx}/platform/bpm/processRun/info.ht?link=1&runId=${bpmProCopytoItem.runId}&copyId=${bpmProCopytoItem.copyId}" title='${bpmProCopytoItem.subject}' class="link detail">详情</a> |
				</display:column>
			</display:table>
