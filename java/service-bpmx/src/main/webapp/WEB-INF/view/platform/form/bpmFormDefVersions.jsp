			...
			<display:table name="versions" id="bpmFormDefItem" requestURI="versions.ht" sort="external" cellpadding="1" cellspacing="1" class="table-grid">
				...
				<display:column title="管理" media="html">
					<c:choose>
						...
						<c:otherwise>
							<a class="link setting" href="setDefaultVersion.ht?formDefId=${bpmFormDefItem.formDefId }&formKey=${bpmFormDefItem.formKey}">
								<span>设置默认</span>
							</a>
						</c:otherwise>
					</c:choose>
					...
					<%-- <c:choose>
						...
						<c:when test="${bpmFormDefItem.designType==1 }">
							<c:if test="${bpmFormDefItem.isPublished==0 }">
								<a onclick="warryInfo()" class="link edit">预览</a>
							</c:if>
							<c:if test="${bpmFormDefItem.isPublished==1 }">
					...
					<c:choose>
						<c:when test="${bpmFormDefItem.designType==0 }">
							<a target="_blank" href="${ctx}/platform/form/bpmFormDef/parse.ht?formDefId=${bpmFormDefItem.formDefId}" class="link preview">终极预览</a>
						</c:when>
						<c:when test="${bpmFormDefItem.designType==1 }">
							<c:if test="${bpmFormDefItem.isPublished==0 }">
								<a onclick="warryInfo()" class="link edit">终极预览</a>
							</c:if>
							<c:if test="${bpmFormDefItem.isPublished==1 }">
								<a target="_blank" href="${ctx}/platform/form/bpmFormDef/parse.ht?formDefId=${bpmFormDefItem.formDefId}" class="link preview">终极预览</a>
							</c:if>
