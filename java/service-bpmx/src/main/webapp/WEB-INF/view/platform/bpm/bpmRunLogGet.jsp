					...
					<tr>
						...
						<td>
							<c:choose>
								...
								<c:when test="${bpmRunLog.operatortype eq 2}"><span class="red">撤销</span></c:when>
								...
								<c:when test="${bpmRunLog.operatortype eq 8}"><span class="red">退回</span></c:when>
								<c:when test="${bpmRunLog.operatortype eq 9}"><span class="red">退回到发起人</span></c:when>
								...
								<c:when test="${bpmRunLog.operatortype eq 19}"><span class="green">更改执行路径</span></c:when>
								<c:when test="${bpmRunLog.operatortype eq 20}"><span class="green">收回任务</span></c:when>
								<c:when test="${bpmRunLog.operatortype eq 21}"><span class="green">结束流程实例</span></c:when>
								<c:when test="${bpmRunLog.operatortype eq 22}"><span class="green">保存草稿</span></c:when>
                                <c:when test="${bpmRunLog.operatortype eq 23}"><span class="green">办结转发</span></c:when>
								<c:otherwise><span class="green">
 退回任意节点</span></c:otherwise>
							</c:choose>
						</td>
					</tr>
					<tr>
