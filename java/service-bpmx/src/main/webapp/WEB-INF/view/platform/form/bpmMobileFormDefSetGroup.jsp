...
var tableId = '${bpmMobileFormDef.tableId}',valid;
...
<script type="text/javascript" src="${ctx}/js/hotent/platform/form/BpmMobileFormDefSetGroup.js"></script>
<body>	
	<div class="panel">
			<div id="teamLayout">
				...
				<div position="center" title="手机表单设置" style="overflow: auto;">
					<div class="panel-toolbar">
						<div class="toolBar">
							<div class="group">
								<a class="link save"  href="javascript:;"><span></span>保存分组并生成表单</a>
							</div>
							<div class="l-bar-separator"></div>
							<div class="l-bar-separator"></div>
							<div class="group">
					...
					<div>
						<div>
							<table cellpadding="1" cellspacing="1"  class="table-detail">
								<tr>
									<th  width="10%">标题:</th>
									<td>
										<input type="text" id="subject" name="subject" value="${bpmMobileFormDef.subject}"  class="inputText"   style="width:250px;">
									</td>
									<th  width="10%">选择生成html模板:</th>
									<td>
										<select name="template" id="template">
										<c:forEach items="${mobileFormTemplate}" var="template">
						...
						<form  id ="mobileForm" method="post" action="${ctx}/platform/form/bpmMobileFormDef/save.ht" >
							<input type="hidden" id="id" name="id" value="${bpmMobileFormDef.id}">
							<input type="hidden" id="formKey" name="formKey" value="${bpmMobileFormDef.formKey}">		
							<textarea style="display: none;" id="groupJson" name="groupJson" >${fn:escapeXml(bpmMobileFormDef.groupJson)}</textarea>						
						</form>
					</div>
				</div>
			</div>
