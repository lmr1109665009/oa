		...
		<c:set var="checkAll">
			...
			<display:table name="processRunList" id="processRunItem" requestURI="myCompletedList.ht" sort="external" cellpadding="1"
				...
				<display:column  titleKey="请求标题" sortable="true" sortName="subject" style="text-align:left">
						${f:subString(processRunItem.subject)}<%-- <c:choose>
				...
				<display:column title="管理" media="html" style="width:100px" class="opsBtnb">
				        <c:choose>
							...
							<c:otherwise>
								<a name="processDetail" onclick="showDetail(this)" href="#${processRunItem.runId}"  action="info.ht?prePage=myCompletedList&link=1&runId=${processRunItem.runId}" title='${processRunItem.subject}' class="link detail">详情</a>
							</c:otherwise>
						</c:choose>
				</display:column>
			</display:table>
