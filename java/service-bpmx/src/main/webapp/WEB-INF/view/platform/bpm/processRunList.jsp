			...
			<div class="panel">
				<div class="panel-top">
					...
					<div class="panel-toolbar">
						<div class="toolBar">
							...
							<div class="group"><f:a alias="delRun" css="link del" action="del.ht"><span></span>批量删除</f:a></div>
							<div class="l-bar-separator"></div>
							...
							<form id="searchForm" method="post" action="list.ht">
									<ul class="row">
										...
										<div class="row_date">
											...
											<li><span class="label label_line">_ </span><input  name="Q_endcreatetime_DG" class="inputText date" value="${param['Q_endcreatetime_DG']}"></li>
										</div>
				...
				<div class="panel-body">
					    ...
					    <display:table name="processRunList" id="processRunItem" requestURI="list.ht" sort="external" cellpadding="1" cellspacing="1"   class="table-grid">
							...
							<!-- 是否有全局流水号 -->
							<c:if test="${hasGlobalFlowNo }">
								<display:column property="globalFlowNo" title="工单号" sortable="true"  sortName="globalFlowNo" style="text-align:center;"></display:column>
							</c:if>
							
							<display:column  title="流程实例标题" media="html" maxLength="18" sortable="true" sortName="subject" style="text-align:center">
								<a href="#${processRunItem.runId}" onclick="FlowDetailWindow({runId:${processRunItem.runId}})" style="text-align:center;" >${processRunItem.subject}</a>
							</display:column>
							<display:column title="流程定义名称" sortable="true" sortName="processName" style="text-align:center">
							...
							<display:column property="creator" title="创建人" sortable="true" sortName="creator" style="text-align:center"></display:column>
							...
							<display:column title="状态" sortable="true" sortName="status">
								<c:choose>
									...
									<c:when test="${processRunItem.status==5}">
										<span class="red">撤销</span>
									</c:when>
									<c:when test="${processRunItem.status==6}">
										<span class="red">退回</span>
									</c:when>
									<c:when test="${processRunItem.status==10}">
										<span class="red">逻辑删除</span>
									</c:when>
								</c:choose>
							</display:column>
