...
<div class="panel">
	...
	<div class="panel-search">
		...
		<input type="hidden" id="isAdmin" name="isAdmin" value="${isAdmin}">
			<ul class="row">
				<li><span class="label" >常用语:</span><input type="text" name="Q_expression_SL"  class="inputText" value="${param['Q_expression_SL']}"/>
				</li><c:if test="${currUserId==1}">
					<li><span class="label" >流程分类:</span>
		    ...
		    <display:table name="taskApprovalItemsList" id="taskApprovalItems" requestURI="list.ht" sort="external" cellpadding="1" cellspacing="1"  class="table-grid">
				...
				<c:choose>
					<c:when test="${isAdmin==1}">
						<c:if test="${taskApprovalItems.userId==currUserId && taskApprovalItems.type!=4}">
						<a href="edit.ht?itemId=${taskApprovalItems.itemId}&isAdmin=${isAdmin}" class="link edit">编辑</a>
						</c:if>
					...
					<c:otherwise>
						<c:if test="${taskApprovalItems.userId==currUserId}">
							<c:if test="${taskApprovalItems.type==4}">
							<a href="edit.ht?itemId=${taskApprovalItems.itemId}&isAdmin=${isAdmin}" class="link edit">编辑</a>
							</c:if>
							<a href="del.ht?itemId=${taskApprovalItems.itemId}" class="link del">删除</a>
						</c:if>
