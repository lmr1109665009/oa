		...
		$.ligerDialog.confirm("确定清除数据?",function(btn){
			...
			$.post('cleanData.ht',{defId:defId},function(data){
	 			...
	 			if(obj.isSuccess()){
	 				...
	 				$.ligerDialog.err('出错信息',"清除数据失败",obj.getMessage());
	 			}
			});
		});
	}
		...
		if(str!=""){
			...
			if(delId==""){
				...
				$.ligerDialog.confirm(str,"消息提示",function(rtn) {
					if(rtn) {
						var form=new com.suneee.form.Form();
	...
	<div class="panel-top">	
				...
				<form id="searchForm" method="post" action="list.ht">	    
					<ul class="row">
						...
						<span class="label">流程分类:</span><input type="text" name="Q_typeName_S" class="inputText" value="${param['Q_typeName_S']}"/>
						 -->
						<%--<li><span class="label" style="width:70px">流程定义Key:</span><input type="text" name="Q_defKey_SL"  class="inputText" value="${param['Q_defKey_SL']}"/></li>--%>
						...
						<li><span class="label">创建时间:</span><input type="text" name="Q_createtime_DL"  class="inputText date" value="${param['Q_createtime_DL']}"/>
		...
		<div class="panel-body">
			    ...
			    <display:table name="bpmDefinitionList" id="bpmDefinitionItem" requestURI="list.ht" sort="external" cellpadding="1" cellspacing="1" export="false"  class="table-grid">
					...
					<display:column property="subject" title="标题" sortable="true" sortName="subject" style="width:10%"></display:column>
					
					<display:column title="分类" sortable="true" sortName="typeId" style="width:12%">
						<c:out value="${bpmDefinitionItem.typeName}"></c:out>
					</display:column>
					<%--<display:column property="defKey" title="FlowKey" sortable="true" sortName="subject"  style="width:12%;"></display:column>--%>
					<display:column property="versionNo" title="版本号" sortable="true" sortName="versionNo" style="width:8%"></display:column>
					<display:column title="发布状态" sortable="true" sortName="status" style="width:12%">
						<c:choose>
