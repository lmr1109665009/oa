<%@ page language="java" contentType="text/html; charset=UTF-8"   pageEncoding="UTF-8"%>
	...
	<head>
		<%@include file="/commons/include/form.jsp" %>
			<title>
				<c:choose>
					<c:when test="${isAdd==1}">添加自定义字段</c:when>
					<c:otherwise>修改自定义字段</c:otherwise>
				</c:choose>
			</title>
				...
				<div class="panel-top">
					...
					<div class="panel-toolbar">
						<div class="toolBar">
							...
							<div class="group">
								<a class="link del" href="javascript:;" onclick="dialog.close();">
									<span></span>关闭</a>
							</div>
						</div>
					</div>
			...
			<form id="frmFields" action="">
				<div class="panel-detail">
					...
					<table class="table-detail" cellpadding="0" cellspacing="0" border="0">
						<tr>
							<th width=100>字段名称:</td>
								<td>
									<input type="text" id="fieldDesc" name="fieldDesc" value="" class="inputText" onblur="autoGetKey(this)" />
								</td>

								<th width=100>字段别名:</td>
									<td>
										<input type="text" id="fieldName" maxlength="20" name="fieldName" value="" class="inputText" />
									</td>
							...
							<th width=100>字段类型:</td>
								<td>
									...
									<span id="spanDateFormat" style="display:none">
										<br> 取当前时间：
										<input type="checkbox" id="isCurrentDate" name="isCurrentDate" value="1">
										<br>
						...
						<!-- <tr>
					<th width=100 >字段类别:</td>
					<td colspan="3">
						<select id="category" name="category">
							<option value="0">表单字段</option>
							<option value="1">流程变量字段</option>
							<option value="2">意见字段</option>
						</select>
					</td>
				</tr> -->
						<tr>
							<th width=100>选项:</td>
								<td colspan="3">
									...
									<c:if test="${isMain==1}">
										<span>
											<input id="isList" name="isList" type="checkbox" checked="checked" value="1" />&nbsp;显示到列表&nbsp;</span>
										<span>
											<input id="isQuery" name="isQuery" type="checkbox" value="1" />&nbsp;作为查询条件&nbsp;</span>
									</c:if>
									<span>
										<input id="isFlowVar" name="isFlowVar" type="checkbox" value="1" />&nbsp;是否流程变量&nbsp;</span>
									<span id="showIsReference" style="display:none;">
										<input id="isReference" name="isReference" type="checkbox" checked="checked" value="1" />&nbsp;作为超连接&nbsp;</span>
									<span>
										<input id="isWebSign" name="isWebSign" type="checkbox" value="1" />&nbsp;是否支持Web印章验证&nbsp;</span>
								</td>
						</tr>
						<!-- 				<tr id="trCondition"  >
					...
					<td colspan="3">
							...
							<th width=100>值来源:</td>
								<td colspan="3">
									...
									<span name="showidentity" id="showidentity">
										...
										<input type="checkbox" name="isShowidentity" id="isShowidentity">是否在页面加载时显示
									</span>
								</td>
						</tr>
						<tr id="trControlType">
							<th width=100>控件类型:</td>
								<td colspan="3">
									<select id="controlType" name="controlType"></select>
								</td>
						</tr>
						<tr id="trDict">
							<th width=100>数据字典类型:</td>
								<td colspan="3">
									<input id="dictTypeName" class="catComBo" catKey="DIC" valueField="dictType" isNodeKey="true" name="dictTypeName" height="150"
									    width="200" />
								</td>
						</tr>
						<tr id="trRule">
						...
						<tr id="associated_fields">
							<th width=100>文件名关联字段:</td>
								<td colspan="3">
									<input type="text" value="" name="associated_fields">
								</td>
						</tr>
						...
						<tr id="trScript">
							<th width=100>脚本:</td>
								<td colspan="3">
									...
									<br>[CUR_USER]：当前人id, [CUR_ORG]：当前人组织id, [CUR_POS]：当前人岗位id。
									<br />
						...
						<tr id="trScriptID">
							...
							<td colspan="3">
								...
								<br/>脚本中要使用到其他字段参与运算， 请使用“[字段名]”方式引用。
								<br />
											...
											<table id="hiddenTable">
												<tbody>
													<tr class="editable-tr">
														<td>
															<div id="optionDiv" class="editable-left-div">
																<label>值:
																	<input name="optionKey" type="text" style="height:21px;" />
																</label>
																<label style="margin:0 0 0 10px;">选项:
																	<input name="optionValue" class="long" type="text" />
																</label>
																<label>默认值:
																	<input name="isDefault" class="checkboxDefault" value="1" type="checkbox" />
																	<input name="isDefault" class="radioDefault" value="1" type="radio" />
																</label>
															</div>
														</td>
														<td>
				...
				<tr id="trUpLoad" style="display:none;">
					...
					<td colspan="3">
						...
						<tr id="fromTypeScriptTr" style="display:none;">
							...
							<td colspan="3">
								<a href="javascript:;" onclick="selectScript()" id="btnScript" class="link var" title="常用脚本">常用脚本</a>
								<br/>脚本中要使用到其他字段参与运算， 请使用“[字段名]”方式引用。返回组织ID
								<br />
						...
						<tr id="showCurUserTr" style="display:none;">
							...
							<td colspan="3">
								<label>
									<input id="showCurUser" name="showCurUser" type="checkbox" />
								</label>
								</th>
						</tr>
						<tr id="showCurOrgTr" style="display:none;">
							...
							<td colspan="3">
								<label>
									<input id="showCurOrg" name="showCurOrg" type="checkbox" />
								</label>
							</td>
						</tr>
						<tr id="showCurPosTr" style="display:none;">
							...
							<td colspan="3">
								<label>
									<input id="showCurPos" name="showCurPos" type="checkbox" />
								</label>
							</td>
						</tr>
					</table>
