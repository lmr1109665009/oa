	...
	<div class="panel">
		...
		<div class="panel-body">
		    ...
		    <display:table name="bpmPrintTemplateList" id="bpmPrintTemplateItem" requestURI="list.ht" sort="external" cellpadding="1" cellspacing="1" class="table-grid">
				<display:column property="temapalteName" titleKey="模版名称" sortable="true" sortName="TEMAPALTE_NAME"></display:column>
				<display:column  titleKey="是否默认" sortable="true" sortName="IS_DEFAULT">
					<c:choose>
				...
				<display:column  titleKey="是否在流程结束后显示" sortable="true" sortName="IS_PRINT_IN_FINISH">
					<c:choose>
						<c:when test="${bpmPrintTemplateItem.isPrintInFinish eq 1}">
							<span class="green">是</span>
						</c:when>
						<c:when test="${bpmPrintTemplateItem.isPrintInFinish eq 0}">
							<span class="red">否</span>
						</c:when>
					</c:choose>
				</display:column>
				<display:column  titleKey="是否显示普通表单" sortable="true" sortName="IS_PRINT_IN_FORM">
					<c:choose>
						<c:when test="${bpmPrintTemplateItem.isPrintInForm eq 1}">
							<span class="green">是</span>
						</c:when>
						<c:when test="${bpmPrintTemplateItem.isPrintInForm eq 0}">
							<span class="red">否</span>
						</c:when>
					</c:choose>
				</display:column>
