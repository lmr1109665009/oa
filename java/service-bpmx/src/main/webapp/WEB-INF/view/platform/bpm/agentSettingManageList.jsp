		...
		<div class="panel-top">
			...
			<div class="panel-search">
				<form id="searchForm" method="post" action="manageList.ht">
					<ul class="row">
						<li>
							<span class="label" style="width:70px">创建人授权人:</span>
							<input type="hidden" id="Q_authid_L" name="Q_authid_L" value="${param['Q_authid_L']}"/>
							<input type="text" name="Q_creator_SL" id="creator"  class="inputText"  value="${param['Q_creator_SL']}" readonly="readonly"/>	
							<input type="button" value="选择..."  onclick="selectUser();" />
						</li>
						<li>
							<span class="label">是否启用:</span>
							<select name="Q_enabled_N" value="${param['Q_enabled_N']}">
								<option value="">所有</option>
								<option value="0" <c:if test="${param['Q_enabled_N'] == '0'}">selected</c:if> >禁止</option>
							...
							<li >	
								<span class="label">结束时间 :</span><input  name="Q_beginenddate_DL"  class="inputText datePicker"  datetype="2" value="${param['Q_beginenddate_DL']}" />
							</li>
		    ...
		    <display:table name="agentSettingList" id="agentSettingItem" requestURI="manageList.ht" sort="external" cellpadding="1" cellspacing="1"  class="table-grid">
				...
				<display:column title="流程名称" sortable="true" sortName="flowname">
					<c:choose>
						...
						<c:when test="${agentSettingItem.authtype==1}"><a href="javascript:;" id="partialAgent">部分流程</a></c:when>
				...
				<display:column title="管理" media="html"  style="width:150px;" class="opsBtnb">
					<a href="editAll.ht?id=${agentSettingItem.id}" class="link edit">编辑</a> |
