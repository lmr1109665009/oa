...
function changeScrollHeight(noNeedFoldHeight,time){
	window.setTimeout(function(){
		...
		//default样式下顶部menu高度
		var defaultMenuHeight = 0; //$(".index_menu",$(parent.document)).height()/2||0;
		/*if(defaultMenuHeight != 0){
			//console.log($(".panel-table").height());
			...
			tableBody.height($(window).height()-panelTtopHeight-panelPage-panelNav-panelToolbar-dialogButtons-panelthead);
			//console.log($(window).height(),panelTtopHeight,panelPage,panelNav,panelToolbar);
		...
		if($(".table-grid.table-list tbody .empty").length){
			...
			$(".table-grid.table-list thead tr th").each(function(index,element){
				...
				$(this).css({"width":currentTable.find("tr td:eq("+index+")").width() });
			  });
			}
        $(function(){
            if($(".table-grid.table-list tbody .empty").length){
		  ...
		  $(window).resize(function(){
			  currentTable.scrollTop(1);//控制滚动条下移10px
			  if( currentTable.scrollTop()>0 ){
			    //alert("有滚动条");
				  currentThead.css({"width": currentTable.width()-7});
			  }else{
			    //alert("没有滚动条");
				  currentThead.css({"width": "100%"});
			  }
			  currentTable.scrollTop(0);//滚动条返回顶部
			  if($(".table-grid.table-list tbody .empty").length){
...
/**ipad上的滚动条**/
function changeIpadScrollHeight(){
		$("body").css("overflow-y","hidden");
		var currentThead=$(".table-grid.table-list thead");
		var currentTable=$(".table-grid.table-list tbody");
		//currentThead.find("tr th:eq(0)").width(currentTable.find("tr td:eq(0)").width());
		...
		//default样式下顶部menu高度
		var defaultMenuHeight = 0; //$(".index_menu",$(parent.document)).height()/2||0;
		var windowHeight=window.sessionStorage.getItem("windowHeight")-110||$(window).height();
		if(defaultMenuHeight != 0){
			//console.log($(".panel-table").height());
			tableBody.height(windowHeight-panelTtopHeight-panelPage-panelNav-panelToolbar-dialogButtons-panelthead);
			//tableBody.height($(window).height()-tableBody[0].offsetTop-defaultMenuHeight-foldHeight-moreHeight);
		}else{
			//console.log($(".panel-table").height());
			//extendIndex样式下顶部menu高度
