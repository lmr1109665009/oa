...
<script type="text/javascript">
	...
	function getOptionData() {
		...
		$("tr.editable-tr", $("#option-table")).each(function() {
			var me = $(this), optionKey = $("input[name='optionKey']", me).val(), optionValue = $("input[name='optionValue']", me).val(), isDefault = $("input[name='isDefault']:checked", me).val();
			if (isDefault != "1") {
				isDefault = "0"
			}
			optionAry.push({
	...
	function addColumn(data) {
		var hiddenTable = $("#hiddenTable"), tmpTr = $("tr.editable-tr", hiddenTable), newTr = tmpTr.clone();
		...
		if (data.key == ' ' || (data.value).indexOf("请选择") > -1) {
			...
			$("input[name='optionValue']", newTr).val(data.value);
			if (data.isDefault == "1") {
				$("input[name='isDefault']", newTr).attr("checked", "checked");
			}
		}
	...
	function init() {
		...
		$("#option-table>tbody").delegate('a.remove-bnt', 'click', function(e) {
			stopBubble(e);
			var me = $(this), tr = me.parents('tr.editable-tr');
			tr.remove();
		});
	...
	function bind(d) {
		for ( var k in d) {
			...
			if (typeof val == 'object' && val != null) {
				...
				if (k == 'options') {
					var options = [], optAry = val.split('#newline#'), opt;
	...
	function getFormatHtml(type) {
		...
		case 'varchar':
			...
			break;
		case 'number':
			html.push('<label for="isShowComdify">千分位:<input id="isShowComdify" prenode="dbType" type="checkbox" eid="isShowComdify" value="1"></label>' + '<label for="isCoin">货币:<input id="isCoin" eid="isCoin" type="checkbox" value="1" onchange="selectCoinChange(this,\'isShowComdify\',\'coinValue\')"></label>' + '<select id="coinValue" prenode="dbType"  eid="coinValue" style="width:80px;display:none"><option value="￥">￥人民币</option><option value="$">$美元</option></select>' + '<label>整数位:<input eid="intLen" prenode="dbType" style="width:30px;" type="text" value="14"/> 小数位:<input eid="decimalLen" prenode="dbType" style="width:25px;" type="text" value="0"/></label>'
					+ '<label>数字范围:<input eid="minValue" prenode="dbType" style="width:30px;" type="text"  value="0"/>,<input eid="maxValue" prenode="dbType" style="width:25px;" type="text" value="0"/></label>');
	...
	dialog.onok = function() {
		...
		$("#inputPanel").find("*:visible").each(function() {
			var id = $(this).attr("eid"), val = $(this).val(), dictType = $("#dictType").val(), prenode = $(this).attr("prenode");
			if ($(this).attr("type")) {
