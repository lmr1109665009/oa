...
<script type="text/javascript">
	...
	$(function() {
		...
		curElement = range.getClosedNode();
		 ...
		 if(!curElement&&isMobileForm&&range){
			 curElement =editor.selection.getStart();
			 if(curElement.nodeName !='INPUT')curElement =$("input",curElement);
		 }
		 if(!curElement) alert("请设置目标计算元素");
	...
	function zTreeOnDblClick(event, treeId, treeNode) {
		...
		var str =isMobileForm?['(data.main.']:['(m:'],
		...
		if(isSingleRecord){
			...
			desc = ['[{'];desc.push(treeNode.fieldDesc);desc.push(str.join(''));desc.push('}]');
		}else{
			desc = ['{'];
			if(!isMain){
				desc = ['['];
