	...
	me.commands['textarea'] = {
			execCommand : function(cmdName,data,old) {
				if(data){					
						...
						if(old){
							old.setAttribute("external",getExternal(data));
							var textarea = $("textarea",old);
							textarea.attr("rows",rowNum);
							textarea.attr("cols",colNum);
						}else{
				...
				if(data){
					...
					for(var i=0,c;c=data[i++];){
						if(c.id=='options'){
							...
							for(var j=0,m;m=options[j++];){
								if(m.isDefault=="1"){
									text.push('<option value="'+m.key+'" selected ="selected">'+m.value+'</option>');
								}else{
...
//					html+='<tr><th>下拉框选项</th><td colspan="3"><span style="color:green;">每行对应一个选项<br /></span><textarea eid="options" cols="50" rows="5"></textarea></td></tr>';
	...
	me.commands['personpicker'] = {
			execCommand : function(cmdName,data,old) {
				if(data){	
					...
					var style=setStyle(data);
					var text='<input type="text" style="'+style+'"/><a href="javascript:;" class="link user">'+content+'</a>';
					if(old){
						old.setAttribute("external",getExternal(data));
						old.innerHTML = text;
					}
	...
	me.commands['rolepicker'] = {
			execCommand : function(cmdName,data,old) {
				if(data){	
					...
					var style=setStyle(data);
					var text='<input type="text" style="'+style+'"/><a href="javascript:;" class="link role">'+content+'</a>';
					if(old){
						old.setAttribute("external",getExternal(data));
						old.innerHTML = text;
					}
	...
	me.commands['positionpicker'] = {
			execCommand : function(cmdName,data,old) {
				if(data){	
					...
					var style=setStyle(data);
					var text='<input type="text" style="'+style+'"/><a href="javascript:;" class="link position">'+content+'</a>';
					if(old){
						old.setAttribute("external",getExternal(data));
						old.innerHTML = text;
					}
	...
	me.commands['departmentpicker'] = {
			execCommand : function(cmdName,data,old) {	
				if(data){
					...
					var style=setStyle(data);
					var text='<input type="text" style="'+style+'"/><a href="javascript:;" class="link org">'+content+'</a>';
					if(old){
						old.setAttribute("external",getExternal(data));
						old.innerHTML = text;
					}
					...
					me.curInputHtml=null;
					var html='<tr><th>按钮文本</th><td><input eid="buttoncontent" type="text" value="选择" /></td><th>其他设定:</th><td><label><input eid="singleselect" type="checkbox" onclick="handCurUser(this)"/>单选</label><label style="display:none"><input eid="showCurOrg" type="checkbox" />显示当前部门</label></td></tr>';
					html+='<tr id="fromTypeTr" ><th>限定类型:</th><td colspan="3"><select id="fromType" name="fromType"></select><div class="tipbox"><a class="tipinfo"><span>根据当前组织，限定选择器的可选择范围</span></a></div></td></tr>';
					html+='<tr id="fromTypeScriptTr" style="display:none;" ><th>脚本:</th><td colspan="3"><a href="javascript:;" onclick="addScript(this)" id="btnScript" class="link var" title="常用脚本">常用脚本</a><br/>返回组织ID<br /> <textarea id="fromScript" codemirror="true" name="fromScript" rows="6" cols="60"></textarea></td></tr>';
					html+=getStyleHtml();
					me.curInputHtml=getContentHtml(html);
	...
	me.commands['subtable'] = {
			execCommand : function(cmdName,data,old) {
				//执行插入html到编辑器的动作
				if(data){
					var num=0,tableName='',tableDesc='',model=0,show='1',parser='';
					for(var i=0,c;c=data[i++];){
						if(c.id=="tablerows")
							num=c.val;
						if(c.id=='tablename')
						...
						if(c.val==1){
							if(c.id=='inlinemodel'){//行模式
								parser='rowmodeedit';
								model=0;
							}
							if(c.id=='blockmodel'){//块模式
								parser='piecemodeedit';
								model=1;
							}
							if(c.id=='windowmodel'){//弹窗模式
								parser='windowmodeedit';
								model=2;
							}
							if(c.id=='inlinemodelNO'){//（序号）行模式
								parser='rowmodeedit';
								model=3;
							}
							if(c.id=='windowmodelNO'){//（序号）弹窗模式
								parser='windowmodeedit';
								model=4;
							}
						}							
					...
					var html=['<div tablename="'+tableName+'" tabledesc="'+tableDesc
					          ...
					          +'" show="'+show+'" external="'+external+'" parser="'+parser+'">'];
	...
	//下面几个在节点消息设置中用到
	//发件人
	me.commands['sendperson'] = {
			execCommand : function() {
				var html = me.getContent();
				me.execCommand('inserthtml',"${发件人}");
			}
	};
	...
	me.commands['subject'] = {
			execCommand : function() {
				var html = me.getContent();
				me.execCommand('inserthtml',"{流程标题:title}");
			}
	};
	
	//发起人
	me.commands['startuser'] = {
			execCommand : function() {
				var html = me.getContent();
				me.execCommand('inserthtml',"{发起人:startUser}");
			}
	};
	
	//开始时间
	me.commands['starttime'] = {
			execCommand : function() {
				var html = me.getContent();
				me.execCommand('inserthtml',"{开始时间:startTime}");
			}
	};
	//开始日期
	me.commands['startdate'] = {
			execCommand : function() {
				var html = me.getContent();
				me.execCommand('inserthtml',"{开始日期:startDate}");
			}
	};
	//业务主键
	me.commands['businesskey'] = {
			execCommand : function() {
				var html = me.getContent();
				me.execCommand('inserthtml',"{业务主键:businessKey}");
			}
	};
	//收件人
	me.commands['readperson'] = {
			execCommand : function() {
				var html = me.getContent();
				me.execCommand('inserthtml',"${收件人}");
			}
	};
	//跳转url
	me.commands['jumpurl'] = {
			execCommand : function() {
				var html = me.getContent();
				me.execCommand('inserthtml',"${跳转地址}");
			}
	};
	//事项名称
	me.commands['taskname'] = {
			execCommand : function() {
				var html = me.getContent();
				me.execCommand('inserthtml',"${事项名称}");
			}
	};
	
	//插入html表单
	me.commands['htmldefform'] = {
			execCommand : function() {
				var html = me.getContent();
				me.execCommand('inserthtml',"${html表单}");
			}
	};
	//插入text表单
	me.commands['textdefform'] = {
			execCommand : function() {
				var html = me.getContent();
				me.execCommand('inserthtml',"${text表单}");
			}
	};
	
	//插入该节点剩余时间
	me.commands['remaintime'] = {
			execCommand : function() {
				var html = me.getContent();
				me.execCommand('inserthtml',"${剩余时间}");
			}
	};
			...
			execCommand : function(cmdName,data,old) {
				if(data){
					if(old){
						...
						insertControl(cmdName,data,'<input type="hidden" '+previewstyle+' />');
					}
				}
				else{					
					dataType=[
	...
	//word插件，将word表格内容转换为html方式。只支持word2007
	var wordFile=null;
	me.commands['word'] = {
			execCommand : function(cmdName,data,old) {
				var url = __ctx + "/platform/word/wordHandler/getHtmlByPath.ht";
				var formHtml = '<form  action="'+url+'" method="post" style="display:none" enctype="multipart/form-data" id="formWord">\
				<input type="file" name="file" id="WordUpload" ></form>';
				...
				wordFile=$("#WordUpload");
				wordFile.bind("change",function(){
					//判断文件名使用docx 结尾。
					var fileName = wordFile.val();
					if(/([^\x00-\xff]|\w)+.(doc|docx)/gi.test(fileName)){
						var frm=$('#formWord');
						frm.ajaxForm({success:function(rtn){
							me.execCommand('insertHtml',rtn);
						} }); 
						frm.submit();
						wordFile.val("");
					}else{
						$.ligerDialog.warn("该按钮只支持word类型文件",'提示');
					}
				});
				wordFile.click();
			}
			...
			execCommand : function(cmdName,data,old) {
				...
				pictureFile.bind("change",function(){
					...
					if(/([^\x00-\xff]|\w)+.(png|gif|jpg|jpeg)/gi.test(fileName)){
						...
						frm.ajaxForm({success:function(rtn){
							var arr = rtn.split('\\');
							var len = arr.indexOf("attachFiles");
							for(var i = 0;i< len ;i++){
								arr.splice(0,1);
							}
							var html = "<img  src='"+ __ctx +"/"+arr.join("/")+"' />";
	...
	function insertControl(cmdName,data,text){
		var html=['<span name="editable-input" '];
		//根据不同控件标上对应的处理器 by liyj
		if(cmdName=="pictureshowcontrol"){//图片控件
			html.push('parser="pictureedit"');
		}
		if(cmdName=="hidedomain"){//隐藏域
			html.push('parser="hiddenedit"');
		}
		if(cmdName=="textinput"){//单行文本框
			html.push('parser="inputedit"');
		}
		if(cmdName=="textarea"){//多行文本框
			html.push('parser="textareaedit"');
		}
		if(cmdName=="checkbox"){//复选框
			html.push('parser="checkboxedit"');
		}
		if(cmdName=="radioinput"){//单选框
			html.push('parser="radioedit"');
		}
		if(cmdName=="selectinput"){//下拉框
			html.push('parser="selectedit"');
		}
		if(cmdName=="dictionary"){//数据字典
			html.push('parser="dictedit"');
		}
		//各种选择器
		if(cmdName=="personpicker"||cmdName=="departmentpicker"||cmdName=="rolepicker"||cmdName=="positionpicker"){
			html.push('parser="selectoredit"');
		}
		if(cmdName=="datepicker"){//日期控件
			html.push('parser="dateedit"');
		}
		if(cmdName=="officecontrol"){//office控件
			html.push('parser="officeedit"');
		}
		if(cmdName=="myeditor"){//富文本框
			html.push('parser="textareaedit"');
		}
		if(cmdName=="attachement"){//附件控件
			html.push('parser="attachmentedit"');
		}
		if(cmdName=="websigncontrol"){//web印章
			html.push('parser=""');
		}
		if(cmdName=="processinstance"){//流程引用
			html.push('parser="proinstedit"'); 
		}
		
		html.push('style="display:inline-block;padding:2px;" class="'+cmdName+'" external="');
