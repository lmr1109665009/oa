		...
		initEditorUI : function() {
			...
			var popup = new baidu.editor.ui.Popup({
				...
				_onInputEditButtonClick : function() {
					...
					if (child) {
						...
						if (external) {
							...
							editor.execCommand(popup.anchorEl.className, false); // xcx
							return;
						}
				...
				_onCustomerDialogClick : function() {
					...
					if (child) {
						editor.curInput = child;
						editor.execCommand("customdialog_mb");
					}
				},
				...
				// ⑨扩展
				// 处理word打印模板
				_onWordTemplateButtonClick : function() {
					this.hide();
					var child = popup.anchorEl;
					if (child) {
						editor.curInput = child;
						editor.execCommand("wordtemplate");
					}
				}
			});
			popup.render();
			if (editor.options.imagePopup) {
				...
				editor.addListener('mouseover', function(t, evt) {
					var el = evt.target || evt.srcElement;
					// 是否手机端表单
					var isNgInput = el.hasAttribute("ht-input") || el.hasAttribute("ht-date") || el.hasAttribute("ht-selector") || el.hasAttribute("ht-textarea");
					...
					if (!isNgInput) {// PC端表单
						// 默认有剪切和删除
						menuHtml += '属性: <span onclick="$$._onInputCutButtonClick(event, this);" class="edui-clickable">剪切</span>&nbsp;&nbsp;';
						...
						if ($(el).is("select")||($(el).is("[external]")&&$(el).children().is("select"))) {// 下拉框
							...
							if ($(el).is("[selectquery]")) {// 已配置了级联，就添加删除级联按钮
								menuHtml += '<span onclick="$$._onUnCascadeQueryClick(event, this);" class="edui-clickable">删除级联</span>&nbsp;&nbsp;';
							}
						}
					} else {// 手机端表单
						// 默认移除按钮
				...
				editor.addListener('selectionchange', function(t, causeByUi) {
					...
					if (editor.ui._dialogs.linkDialog) {
						...
						var isWordTemplate = false;
						var url;
						if (link) {
							var wordTemplateAttr = link.getAttribute("templateAlias");
							if (wordTemplateAttr) {
								isWordTemplate = true;
							}
						}
						...
						if (isWordTemplate) {
							html = popup.formatHtml('<nobr>' + ': <span onclick="$$._onWordTemplateButtonClick();" class="edui-clickable">修改</span>&nbsp;&nbsp;</nobr>');
							popup.showAnchor(link);
						} else if (link && (url = (link.getAttribute('data_ue_src') || link.getAttribute('href', 2)))) {
							var txt = url;
							if (url.length > 30) {
							...
							html += popup.formatHtml('<nobr>' + editor.getLang("anthorMsg") + ' <span class="edui-clickable" onclick="$$._onEditButtonClick();">' + editor.getLang("modify") + '</span>' + ' <span class="edui-clickable" onclick="$$._onRemoveButtonClick(\'unlink\');"> ' + editor.getLang("clear") + '</span></nobr>');
							popup.showAnchor(link);
						}
					}
