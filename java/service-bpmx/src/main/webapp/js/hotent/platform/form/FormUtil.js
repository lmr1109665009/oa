...
FormUtil.initCommonDialog=function(){
	...
	$("a.extend",$(".formTable")).each(function(){      //只读时，历遍超链接中自定义对话框的按钮
		...
		if(jsonStr != null && 'undefined' != jsonStr && jsonStr.length>2 ){
        ...
        6:function(c){//非必填
			...
			if(FormUtil.isAttachment(self)){
				...
				if(validate){
					...
					self.removeClass("validError");
					if(self.hasClass("Wdate")){
						//self.css("border","1px solid #E8EEF0");
					}
				}
			}
			FormUtil.linkageShow(self);
...
FormUtil.InitGangedSet = function(d){
	...
	for(var i=0,c;c=d[i++];){
		...
		for(var j=0,m;m=choisefield[j++];){
			...
			var isEnable=changeObj.prop("disabled");
			changeObj.prop("disabled",false);
			changeObj.trigger("change");
			changeObj.prop("disabled",isEnable);
		}
	}
 ...
 * 初始化office按钮
 */
FormUtil.initOfficeButton = function(){
	var pathName = window.location.pathname;
	if(pathName.indexOf('OfficeTemplate') > -1){
		return;
	}
	this.fileObjs=$("input[controltype='office']");
	for ( var i = 0; i < this.fileObjs.length; i++) {
		var fileObj = this.fileObjs.get(i);
		var fileId = $(fileObj).val();
		if(fileId.length > 0){
			var path = __ctx +'/platform/system/sysFile/getFile.ht';
			$.post(path,{fileId:fileId},function(data){	
				// var buttons = '<span class="attachement-span"><a class="attachment"  href="##" onclick="openOffice(this,'+fileId+')">'+data.fileName+'.'+data.ext+'</a></span>';
				var buttons = '<span class="attachement-span"><a class="attachment"  href="##" onclick="openOffice(this,'+fileId+')">'+data.fileName+'</a></span>';
				// 提取绑定字段
				var associatedFields =  $(fileObj).attr("associated_fields");
				if(associatedFields){
					var right = permission.field[associatedFields]
				    if(right == 'b' || right == 'w'){
					   buttons += '<a class="link reload" style="border:1px solid #dddddd" onclick="reloadDocName(this)"><span>更新文档名称</span></a>'
				    }
				}
				   $(fileObj).after(buttons);
			 });
		}else if ($(fileObj).attr("right")=='w'||$(fileObj).attr("right")=='b'){
            var buttons = "<a id='officeButton' class='link add' href='##' onclick='openOffice(this)'><span></span>新建</a>";
            $(fileObj).after(buttons);
        }
	}
};
FormUtil.delFile = function(obj){
	$.ligerDialog.confirm('确定要删除', function (rtn){
		if(rtn){
			var url = __ctx + "/platform/system/sysFile/delByFileId.ht";
			var inputObj=$(obj);
			var parent=inputObj.parent();
			$.post(url,{ids:parent.prev("input[controltype='office']").val()},function(data){
				if(data.success == "true"){
					parent.prev("input[controltype='office']").val("");
					parent.next("a.reload").remove();
					//删除span
					parent.remove();
					$("#officeButton").show();
...
function openOffice(obj,fileId){
	var url=__ctx+"/platform/form/OfficeTemplate.ht";
	var menuRight = "";
	var inputObj;
	if(fileId){
		inputObj = $(obj).parent().siblings("input[controltype='office']");
	}else{
		inputObj = $(obj).siblings("input[controltype='office']");
	}
	var ziduan = inputObj.attr("name").split(":")[2];
	if(permission){
		menuRight = permission.menuRight[ziduan];
        if (menuRight){
            menuRight=JSON.stringify(menuRight);
        }
	}
	var right = inputObj.attr("right");
	// 提取绑定字段
	var associatedFields =  inputObj.attr("associated_fields");
	var isBinding = false;
	if(associatedFields && associatedFields != ""){
		// 有绑定字段
		isBinding = true;
	}
	var fileds = inputObj.attr("name").split(":");
	var finame = ""
	if(isBinding){
		var bindingName = fileds[0]+":"+fileds[1]+":"+associatedFields;
		finame = $("input[name='"+bindingName+"']").val();
		if(finame == ""){
			var title=$("input[name='"+bindingName+"']").attr("lablename");
			$.ligerDialog.warn("请先填写"+title+"!","提示");
			return;
		}
	}
	if(fileId){
		url += "?fileId="+fileId + "&menuRight="+encodeURIComponent(menuRight)+"&right="+right;
	}else{
		url += "?menuRight="+encodeURIComponent(menuRight)+"&right="+right;
	}
	if(finame != ""){
		url += "&isBinding="+isBinding +"&bindingName="+bindingName;
	}
	$.openFullWindow(url);
}

function getfiname(bindingName){
	return $("input[name='"+bindingName+"']").val();
}
function reloadDocName(obj){
	var inputObj = $(obj).siblings("input[controltype='office']");
	// 提取绑定字段
	var associatedFields =  inputObj.attr("associated_fields");
	var isBinding = false;
	if(associatedFields && associatedFields != ""){
	...
	if(isBinding){
		...
		finame = $("input[name='"+bindingName+"']").val();
		if(finame == ""){
			$.ligerDialog.warn("绑定字段"+$("input[name='"+bindingName+"']").attr("lablename")+"不能为空","提示");
			return;
		}
		//开始更新文件名
		var fileId = inputObj.val();
		var path = __ctx +'/platform/system/sysFile/updateFileNameByFileId.ht';
		$.post(path,{fileId:fileId,filename:finame},function(data){	
			   if(data.success == "true"){
				   $(obj).siblings(".attachement-span").children(".attachment").text(finame+".doc");
			   }else{
				   $.ligerDialog.warn("更新文档名称失败","提示");
			   }
		 });
	}
}
function addNewDoc(fileId,fileName){
	this.fileObjs=$("input[controltype='office']");
	for ( var i = 0; i < this.fileObjs.length; i++) {
		var fileObj = this.fileObjs.get(i);
		if($(fileObj).val()==fileId){
			continue;
		}
		var buttons = '<span class="attachement-span"><a class="attachment"  href="##" onclick="openOffice(this,'+fileId+')">'+fileName+'</a><a href="javascript:;" onclick="FormUtil.delFile(this);" class="cancel"></a></span>';
		// 提取绑定字段
		var associatedFields =  $(fileObj).attr("associated_fields");
		if(associatedFields){
			var right = permission.field[associatedFields]
...
/**
 * 初始化textarea 
 * 插件描述：可设置最小的高度，之后随着内容增加自动延伸文本框高度.
 */
