...
function add(t,o){
	...
	if(!t){
		...
		$("[name='teamTitle']", table).val(o.teamTitle);
...
function selectTeam (obj){
	...
	$(obj).addClass('validError validYz');
...
BpmFormTableTeam.addField = function(treeId,treeNode) {
	...
	if(flag){
        var dialog = $.ligerDialog.confirm('该字段已经添加分组,请先移除再添加!','提示信息',function(rtn) {
            dialog.close();
		});
		return;
	}
	if(!selectTeam||selectTeam.length==0){
        var dialog = $.ligerDialog.confirm('请选择要添加的分组！','提示信息',function(rtn) {
            dialog.close();
        });
		return;
	}
...
function saveData(){
	...
	$("#teamItem .table-detail").each(function(){
		...
		data.teamTitle = $("input[name='teamTitle']",me).val();
