...
FormMath.doMath = function(){
	...
	$("input[funcexp]").each(function(){
		...
		try{
			...
			var tempVal = value+"";
			if(tempVal.length>10&&tempVal.indexOf("\.")>0)
				var showtype = JSON.parse( $(me).attr("showtype"));
				for(var i = 0 ; i < 10;i++){
					var k = "";
					for(var j = 0 ; j <6 ; j++)
						k = k + i;
					if(tempVal.indexOf(k)>0){
						value = value.toFixed(showtype.decimalValue);
						break;
					}
				}
		}
		...
		if(/^[0-9]*$/.test(value)){
			...
			if(jsonValidate  && jsonValidate.length>0){
...
FormMath.replaceSingleValue = function(exp,t){
	...
	exp = exp.replace(reg,function(){
		...
		//子表
		if(/^s\:.+$/.test(name)){
			var pTr = $(t).closest(".listRow,.block");
