...
function init(){
    $('[ctlType="flowSelector"]').each(function(){
        ...
        $('[ctlType="flowSelector"]').parent().append($(".reset:first"));
    ...
    $('[ctlType="selector"]').each(function(){
        ...
        var siblingsId = $(this).attr('name')+'ID';
        var oldVal=$('[name="'+siblingsId+'"]').val();
        // 处理某个事件的代码 改变input的值 清空同胞[name=siblingsId]的值
        $(this).on('input change', function() {
            var name = $('[name="'+siblingsId+'"]');
            if(!name.hasClass("changeaa")){
                name.val('');
                if($(this).val()==$(this).attr('title')){
                    name.val(name.attr('oldval')||oldVal);
                }
                console.log($(this).val()==$(this).attr('title'))
                console.log($(this).val(),$(this).attr('title'),name.val(),oldVal)
            }
        });
...
function buildSelector(obj, className){
    var self = $(obj);
    self.wrap('<div style="display: inline-block;"></div>');
...
function handSelector(){
    ...
    $("body").delegate("a.link.org", "click",function(){
	  ...
	  //10.公司选择器
    $("body").delegate("a.link.comp", "click",function(){
        selector($(this),10);
    });
    // 6.组织选择器（多选）
    ...
    $("body").delegate("a.link.reset", "click",function(){
        ...
        var textFilter="div[actinststr='"+fieldName+"']";
...
function showDetailactInst(obj){
    var url = $(obj).attr("action");
    jQuery.openFullWindow(url);
};
function triggerChange(inputId,inputName,oldIdVal,type,fieldName){
//	inputName.focus();
    ...
    inputId.removeClass("changeaa");
    inputName.attr("title",inputName.val());
    inputId.attr("oldval",inputId.val());
    if(type==9){
        $("input[name='"+ fieldName + "']").hide();
        var actInstsArr=inputName.val().split(',');
        var actInstStr='';
        var actInstid=inputId.val().split(',');
        for(i=0;i<actInstsArr.length;i++){
            actInstStr+='<P><a onclick="showDetailactInst(this)" style="color: #000;" href="javascript:;" action="/bpmx/platform/bpm/processRun/info.ht?link=1&runId='+actInstid[i]+'">'+ actInstsArr[i]+'</a></P>'
        }
        var actInstStrName =$("div[actInstStr='"+ fieldName + "']");
        if(actInstStrName.length){
            actInstStrName.remove();
        }
        inputId.parent().prepend('<div actInstStr='+fieldName+'>'+actInstStr+'</div>');
        //actInstStrName.prepend(actInstStr);
        //console.log(actInstStrName.html(),actInstsArr.length);
    }
...
function selector(obj, type) {
    ...
    var siblingsId = obj.attr('name')+'ID';
    var name = $('[name="'+siblingsId+'"]');
    name.addClass("changeaa");
    //1毫秒内同时打开两次不允许 by xg
        ...
        case 9 :
            ...
            ActInstDialog({
                ...
                callback : function(data) {
                    ...
                    triggerChange(inputId,inputName,oldIdVal,9,fieldName);
                }
            });
    }
