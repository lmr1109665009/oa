...
PageTab = function(divContainerId,total,conf){
	{
		...
		var imgPlus=__ctx +"/styles/default/css/tab/icon_plus.gif";
		
		var imgDelete=__ctx +"/styles/default/css/tab/icon_minus.gif";
		...
		$("#pageList").delegate("li>b", "dblclick", function() {
				...
				txtObj.blur(function(){
					var tmpObj=$(this);
					var val=tmpObj.val();
					if(_self.isExistPageTitle(val)){
						$.ligerDialog.warn('该名称已经被使用，请重新输入',"提示信息");
						return;
					}
					_self.pageTitle = val;
				...
				txtObj.keydown(function(event) {
					 if (event.keyCode == '13') {
						 var val = txtObj.val();
						 if(_self.isExistPageTitle(val)){
							alert('该名称已经被使用，请重新输入',"提示信息");
							return;
						 }
						 _self.pageTitle = val;
						 txtObj.parent().text(val);
						 txtObj.remove();
						 var curPage=_self.getCurrentIndex();
						 _self.conf.txtCallBack(curPage);
					 }
	...
	this.addPage =function(){
		this.totalPage ++;
		var i = this.totalPage;
		while(this.isExistPageTitle("新页面"+i)){
			i++;
		}
		this.pageTitle = "新页面"+i 
		var li=$("<li><b>"+this.pageTitle+"</b></li>");
		this.attachTabEvent(li);
	 ...
	 * 判断是否已经存在title
	 */
	this.isExistPageTitle = function(newTitle){
		var tabs = $("b",$("#pageList"));
		for(var i = 0 ; i < tabs.length ; i++ ){
			if(tabs[i].textContent == newTitle){
				return true;
			}
		}
		return false;
