...
OfficePlugin={
		...
		init:function(config){
			...
			for ( var i = 0; i < this.fileObjs.length; i++) {
				...
				//获取附件的扩展名
				var doctype = fileObj.getAttribute("doctype");
				...
				if(right=="no"){
					...
					var finalConfig=$.extend(config,{fileId:fileId,doctype:doctype,myNum:myNum,right:right,user:this.user,menuRight:menuRight});
					officeObj.renderTo(divId,finalConfig);
					//是否有office控件。
		...
		submit:function(officename,callBack,ext){
			//每次提交，都需要清零
			this.submitNewNum = 0;
			if (ext){
                this.officeName = officename+"."+ext;
			}else {
                this.officeName = officename+".doc";
			}
			for ( var cn = 0; cn < this.officeObjs.length; cn++) {
				...
				if(right=="w"||right=="b"){
					//保存到服务器。
					var result=officeObj.saveRemote(cn,officename);
					if(result==-11){
                        ...
                        $.ligerDialog.error("提交失败,OFFICE控件没能正常使用，请重新安装 ！！！","提示",function () {
						...
						if(result>0){
							...
							this.hasSubmitOffices[cn]=true; //完成标志
							this.submitNewNum = this.submitNewNum + 1;  //文档是提交了的数目     变化的(重新清空)
							if(!OfficePlugin.isExist){
								window.opener.addNewDoc(result,OfficePlugin.officeName);
							}
							if (OfficePlugin.isNeedClose==true){
                                window.close();
							}
						}else{
							break; //文件上传失败
		...
		hiddenObj:function(str){
			if(str == true){
				$("object[id^='office_div']").css("height","1px");
			}else{
				$("object[id^='office_div']").css("height","100%");
			}
		}
};
