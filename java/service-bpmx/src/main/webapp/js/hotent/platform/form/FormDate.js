...
FormDate.doDateCalculate = function(obj){
	$("input[datecalculate]").each(function(){
		...
		var	dateCalculate = me.attr("dateCalculate");
...
FormDate.getDateDiff = function (dateCalculate,t){
	...
	if(isMain){
		...
		if(typeof pTr =="undefined" || pTr.length <=0 ){
			pTr = $(t).closest(".listTable");
		}
 ...
 * 返回times 差
 */
FormDate.compareDate=function(dateA,dateB){
	dateA = dateA.replace(/\-/g, "/");
	dateB = dateB.replace(/\-/g, "/");
	var sTime = new Date(dateA);
	var eTime = new Date(dateB); 
