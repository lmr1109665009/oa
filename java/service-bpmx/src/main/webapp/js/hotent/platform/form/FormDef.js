...
FormDef.insert = function (editor, node,templateAlias){
	$.post('getControls.ht', {
		...
		var template=$(controls[node.fieldName]);
        var label=template.attr("lablename");
        if (!label||label==""){
            label=template.attr("opinionname");
        }
        if (!label||label==""){
            label=template.attr("tabledesc");
        }
        if(!label){
            label="";
        }
        template.attr("placeholder",label);
		editor.execCommand('inserthtml',$("<div></div>").append(template).html(),1);
	});
};
/**
 * 选择模板
