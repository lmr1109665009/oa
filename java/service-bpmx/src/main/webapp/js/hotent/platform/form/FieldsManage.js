...
FieldsManage=function(){
	 ...
	 * 取得复选框。
	 */
	this.getCheckBoxDis=function(field,optionType){
		return "<input type='checkbox' name='"+optionType+"'  "+((field[optionType]==1)?"checked":"")+" disabled='disabled'/>";
	};
	/**
	 * 取得字段类型。
	 */
	...
	this.getHtml=function(conf){
		...
		for(var i=0;i<this.Fields.length;i++){
			...
			// sb.append("<td style='text-align:center;'>"+this.getCheckBoxDis(field,"isRequired")  +"</td>");
			...
			sb.append("<td style='text-align:center;'>"+this.getCategory(field.category) +"</td>");
...
//				sb.append("<td style='text-align:center;'>&nbsp;</td>");
//			}
			sb.append("<td>" +((field.isDeleted==0)?"<a href='#"+field.fieldName+"' name='editColumn'  >编辑</a>":" ")+
					"&nbsp;&nbsp;<a href='#' onclick='TableRow.delByFieldName(this,&#39;"+field.fieldName+"&#39;)'>删除</td>");
			sb.append("</tr>");
		}
		return sb.toString();
	};
 ...
 * 删除行
 */
TableRow.delByFieldName=function(obj,fieldName){
	// var outerWindow = window.top;
	var outerWindow = window.parent;
	outerWindow.$.ligerDialog.confirm('确定要删除吗?','提示', function(rtn) {
		if(!rtn) return ;
		var objTr=$(obj).closest("tr");
		var field=TableRow.fieldManage.getFieldByName(fieldName);
		if(field.isRequired){
			outerWindow.$.ligerDialog.warn('该列为必填列,不能删除!','提示');
			return;
		}
		TableRow.fieldManage.delField(fieldName);
		objTr.remove();
