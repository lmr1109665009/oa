...
var controlList = [ {key : '1',value : '单行文本框'}, {key : '2',value : '多行文本框'},
                    {key : '10',value : '富文本框'},{key:'24',value:'多行文本框(HTML)'}, {key : '3',value : '数据字典'},
...
function getField(){
	...
	if(field.controlType==12){
		//associated_fields office控件产生的文件名是否与某个字段的值关联
		var associated_fields = $("input[name='associated_fields']").val();
		var json={"associated_fields":associated_fields};
...
//	$("#ccisread").attr("checked",field.ccisread==1);
	...
	hdlIsWebSignShow(ft,vf,ct);
	hdlShowAssociated_fields(ft,vf,ct);	
	...
	if(field.ctlProperty){
		...
		$("#isShowidentity").each(function(){
			$("#isShowidentity").attr("checked",isShowidentity==1);
		});
		$("input[name='associated_fields']").each(function(){
			$("input[name='associated_fields']").val(prop.associated_fields);
		});
	}
...
function bindTable(data,allowEditTbColName,mainTableIsPublished){
	...
	$("#categoryId").val( table.categoryId);
	$(".catComBo").val(table.categoryName);
	...
	case varchar_://文本
		...
		case '0'://表单输入
			...
			controlList =[ {key : '1',value : '单行文本框'}, {key : '2',value : '多行文本框'},
                     {key:'10',value:'富文本框'},
					{key:'24',value:'多行文本框(HTML)'},{key : '3',value : '数据字典'},
                    {key : '4',value : '人员选择器(单选)'}, {key : '8',value : '人员选择器(多选)'}, 
                    {key : '17',value :'角色选择器(单选)'},{key : '5',value : '角色选择器(多选)'}, 
                    {key : '18',value : '组织选择器(单选)'},{key : '6',value : '组织选择器(多选)'},
                    {key : '25',value : '公司选择器'},
		...
		case '2'://脚本输入 (隐藏)
			controlTypeTr.show();
			controlList =  [ {key : '1',value : '单行文本框'}, {key : '2',value : '多行文本框'},{key:'24',value:'多行文本框(HTML)'},
		                  	 {key : '3',value : '数据字典'}, 
		                    {key : '4',value : '人员选择器(单选)'}, {key : '8',value : '人员选择器(多选)'}, 
		                    {key : '17',value :'角色选择器(单选)'},{key : '5',value : '角色选择器(多选)'}, 
		                     {key : '18',value : '组织选择器(单选)'},{key : '6',value : '组织选择器(多选)'},
...
function hdlShowAssociated_fields(fy,vf,ct){
	if(ct==12){
		$("#associated_fields").show();
	}else{
		$("#associated_fields").hide();
	}
}
