...
function serializeObject(form){
	...
	$(".js-example-basic-multiple").each(function(){
		var obj = $(this);
		var objects = obj.select2("data");
		if(objects.length == 0){
			return true;
		}
		var valueArray = [];
		for(var i = 0 ; i < objects.length ; i++){
			valueArray.push(objects[i].id);
		}
		var name = $(this).attr("fieldname");
		o[name] = valueArray.join(",")
	});
	return o;
}
...
function openLinkDialog(conf){
	...
	var	width= conf.width||800;
	var	height=conf.height||600;
	var	isFull =conf.isFull||false;
	var	title =conf.title||'新增';
	var	isStart = conf.isStart||false;
	var param = conf.param||{};//参数
	if(isFull){
