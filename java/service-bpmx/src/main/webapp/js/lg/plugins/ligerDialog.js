...
{
    var l = $.ligerui;
    var windowHeight = window.sessionStorage.getItem("windowHeight")||$(window).height();
    //全局事件
    ...
    l.controls.Dialog.ligerExtend(l.core.Win, {
        ...
        _render: function ()
        {
            ...
            if (p.allowClose == false) $(".l-dialog-close", g.dialog).hide();
            ...
            {
            	p.target.dialog = g;
            	g.dialog.content.addClass("l-dialog-content-center");
            ...
            if(p.isPostHidden){
                g.iframe.parent("body").css({"overflow-y" : "hidden" });
            }
            //设置按钮
            if (p.buttons)
            {
            ...
            if (!p.fixedType)
            {
                ...
                else p.top = top = 0.5 * (windowHeight - g.dialog.height()) + $(window).scrollTop() - 10;
        ...
        {
            var g = this, p = this.options;
            var width = $(window).width(), height = windowHeight, left = 0, top = 0;
            if (l.win.taskbar)
            {
        ...
        close: function ()
        {
            ...
            g.iframe.hide();
            if(p.isPostHidden){
                g.iframe.parent("body").css({"overflow-y" : "visible" });
            }
        },
            ...
            if ($.fn.ligerDrag)
                g.draggable = g.dialog.ligerDrag({ handler: '.l-dialog-title', animate: false,
                    ...
                    onDrag: function (current, e)
                    {
                    	...
                    	if(pageX<=30||pageY<=30||pageX>=$(window).width()-30||pageY>=windowHeight-30){return false}
                    },
                    onStopDrag: function ()
                    {
                        if (p.target)
    ...
    $.ligerDialog.alert = function (content, title, type, callback,width,height)
    {
        ...
        return outerWindow.$.ligerDialog(p);*/
    };
    ...
    $.ligerDialog.confirm = function (content, title, callback)
    {
        ...
        p = {
            type: 'question',
            content: content,
            buttons: [{ text: $.ligerDefaults.DialogString.ok, onclick: btnclick, type: 'ok' }]
        };
        if (typeof (title) == "string" && title != "") p.title = title;
        $.extend(p, {
            showMax: false,
    ...
    $.ligerDialog.waitting = function (title)
    {
        ...
        return $.ligerDialog.open({ cls: 'l-dialog-waittingdialog', type: 'none', content: '<div style="padding:4px">' + title + '</div>',allowClose:false,showMax:false,showToggle:false,showMin:false});
    };
    ...
    $.ligerDialog.waittingCH = function (title)
    {
        title = title || $.ligerDefaults.Dialog.waittingMessage;
        return $.ligerDialog.open({ cls: 'l-dialog-waittingdialogCH', type: 'none', content: '<div style="padding:4px">' + title + '</div>', allowClose: true });
    };
    
    $.ligerDialog.closewaittingCH = function ()
    {
        var dialogs = l.find(l.controls.Dialog);
        for (var i in dialogs)
        {
        ...
        //  $(".l-window-mask").remove();
            if (d.dialog.hasClass("l-dialog-waittingdialogCH"))
                d.close();
        }
    };
   ...
   $.ligerDialog.tipDialog = function (title,msg,detail,type,onBtnClick)
    ...
    {    
    	if (!this.windowMask)
        {
            this.windowMask = $("<div class='l-window-mask l-window-mask2' style='z-index:9200;display: block;'></div>").appendTo('body');
        }
    	$(".l-window-mask2").show();
        content = content || "";
        if (typeof (title) == "function")
