...
(function($) {
	...
	$.fn.rowOps = function() {
		...
		/*//widthall根节点
        function widthall(){
        	var length= me.children("li").length;
        	var li=me.children("li");
        	var widthall;
		...
		me.find("a").each(function(i,n) {
			...
			if (typeof isCommon == "undefined" || isCommon != "true") {
				...
				textLength = aText.length>0?(aText.length*12+14):0;
				//字的宽度+偏移的宽度+图标的宽度
				widthed +=  (textLength+20+12);	
				if(i+1==num){
