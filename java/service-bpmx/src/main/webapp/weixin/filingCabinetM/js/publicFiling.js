/**
 * Created by xucuiyun on 2017/5/16.
 */
$(function(){
                ...
                if (data.length == 0) {
                    ...
                    if (!data[0].documentIPath) {
                        ...
                        for (var i = 0; i < data.length; i++) {
                            str += '<div class="oneList" onclick=' + 'getClickData(' + data[i].id + ',' + '"' + data[i].documentName + '"' + ',' + data[i].parentId + ')><div class="oneList-left fl"><p class="docName">' + data[i].documentName + '</p><p class="createTime">' + data[i].uploadTime + '</p></div><div class="oneList-right fr"><p class="creator">' + data[i].creatorName + '</p></div></div>'
                            $('#fContent_mobile').html(str);
                        }
                    } else {
                        //文件在页面的渲染方式
                        if(isiOS){
                            for (var i = 0; i < data.length; i++) {
                            	str += '<div class="oneList clearfloat"><div class="oneList-left fl"><p class="docName">' + data[i].documentName + '</p><p class="createTime">' + data[i].uploadTime + '</p></div><div class="oneList-right fr"><p class="creator">' + data[i].creatorName + '<a target="_blank" class="operate" onclick=' + 'downloadWJ('+ '"' + url1 + '?sessionId='+ userName +'&documentIPath=' + data[i].documentIPath +'&id=' + data[i].id + '&openIt=true'+'"'+')><img src="images/download.png"></a></p></div></div>'
                            }
                            $('#fContent_mobile').html(str);
                        }
                        else if(isAndroid){
                        	for (var i = 0; i < data.length; i++) {
                            	str += '<div class="oneList clearfloat"><div class="oneList-left fl"><p class="docName">' + data[i].documentName + '</p><p class="createTime">' + data[i].uploadTime + '</p></div><div class="oneList-right fr"><p class="creator">' + data[i].creatorName + '<a target="_blank" class="operate" onclick=' + 'getOutBrower('+ '"' + url1 + '?sessionId='+ userName +'&documentIPath=' + data[i].documentIPath +'&id=' + data[i].id +'"'+')><img src="images/download.png"></a></p></div></div>'
                            }
                            $('#fContent_mobile').html(str);
                        }
                        else{
                       for (var i = 0; i < data.length; i++) {
                           str += '<div class="oneList"><div class="oneList-left fl"><p class="docName">' + data[i].documentName + '</p><p class="createTime">' + data[i].uploadTime + '</p></div><div class="oneList-right fr"><p class="creator">' + data[i].creatorName + '<a target="_blank" class="operate" href="' + url1 + '?sessionId='+userName+'&documentIPath=' + data[i].documentIPath +'&id=' + data[i].id + '"><img src="images/download.png"></a></p></div></div>'                        }
                        $('#fContent_mobile').html(str);
                       }
                    }
                }
