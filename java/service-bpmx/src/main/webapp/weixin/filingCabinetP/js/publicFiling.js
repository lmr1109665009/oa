/**
 * Created by xucuiyun on 2017/5/15.
 */
        ...
        $.ajax({
            ...
            success:function(result) {
                ...
                if (data.length == 0) {
                    ...
                    str += "<tr><td colspan=4>此文件夹为空</td></tr>"
                    ...
                    if (!data[0].documentIPath) {
                        ...
                        $('th').css('width', '20%');
                        for (var i = 0; i < data.length; i++) {
                            str += '<tr><td class="docName">' + data[i].documentName + '</td><td>' + data[i].docuemntSize + '</td><td>' + data[i].uploadTime + '</td><td>' + data[i].creatorName + '</td><td><a href="' + url1 + '?sessionId='+userName+'&documentIPath=' + data[i].documentIPath +'&id=' + data[i].id + '">下载</a></td></td></tr>';
                        }
                        $('#fContent_tbody').html(str);
                        $('th').css('width', '15%');
                        $('th:nth-child(1)').css('width', '40%');
