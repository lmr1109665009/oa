...
function getRegionData(){
	...
	$.get(url,function(result){
		...
		if(!result.data && typeof(result.data)!="undefined" && result.data!=0){
			...
			else if(result.data.length!=0){var dicId=result.data[0].dicId;checkRegion(dicId)	}
			else{ }
		}
		else {
			//console.log(result.message); 
...
function getSbData(){  
		...
		$.get(url,function(result){
			...
			$("#tpi-checkbox-sb-box").html("<br/><br/><br/><center>"+result.message+"<br/>请先选择你的会议时间!</center>");	
			//console.log(result.message); 
			}
		})
	}
//获取系统用户信息
  function getUserData(){  
		...
		$.get(url,function(result){
			...
			$(".chosen-select option[value='10000001274471']").attr("selected","selected");  
			$(".chosen-select").chosen({
				  no_results_text: "无法搜索",  
			      });		
			}
			else {
				$(".mroom-wrap #tpi-ry-box").append("<center>"+result.message+"</center>");
				//console.log(result.message);
				//$(".chosen-select").html('<select data-placeholder="+添加人员" class="chosen-select chzn-done" multiple="" id="selJAB" style="display: none;"> <option value="10000001274471" dataname="袁婷">袁婷 dianmo@suneee.com</option>   <option value="10000001063141" dataname="测试">测试 4455@qq.com</option></select>');
				/*$(".chosen-select").chosen({
					  no_results_text: "无法搜索",
				      });*/
...
function initDateY()
{
  $(".axis-y-date").each(function(i, e){
        $(e).html(bookerDate[i]);
    });

}

$(window).resize(function(){lunBoDateY()});
function lunBoDateY()
{
...
me":"会议室一","convoketDateText":null,"createByName":"超级管理员"}]
  ...
  $("<div class='prev'>&nbsp;</div><div class='next'>&nbsp;</div>").appendTo(c.renderTo);
  ...
  for(var j=0;j<bookerIntDate.length;j++)
    ...
    var ela = $('<div class=container style=z-index:1'+j+'>').addClass(c.line % 2 ? "odd" : "even");
      ...
      var cls = ''; 
            ...
            var clhtml='<span  class="'+cls+' cltd" date1='+date1+' date2='+date2+' i='+i+'><i style="border-right: solid 1px #eaeaea;">'+date1+'<br/>'+date2+'</i></span>';
            clul.append(clhtml);
          };
    cltr.append(clul);
  if(meetings.length>0){
        for(var b=0;b<meetings.length;b++){
            ...
            var clhtml;
            //var tipbox=$("<div class=tip_box title="+ m_proposer+" data-am-popover={content: "+ m_proposer+"}></div>");
            if(intday==j){
              ...
              for(var i=0;i<clheight;i++){   
               ...
               clhtml='<span class="'+ cls +' cltd"></span>';ul.append(clhtml);}
                ...
                if(intTimeD <= i && i < intTimeDiff){  
                    ...
                    var date1 = minsTime(i*30); 
                    var date2 = minsTime(i*30+30);
                     if (intTimeD == intTimeDiff-1){clhtml='<span class="'+ cls +' disabledd linedd cltd"><i><span>'+ m_proposer+'</span>'+date1+'<br/>'+date2+'</i></span>'}
                     else if(intTimeD == i){ clhtml='<span class="'+ cls +' disabledd cltd"><i><span>'+ m_proposer+'</span>'+date1+'<br/>'+date2+'</i></span>' }
                    else if(intTimeDiff-1 == i){ clhtml='<span class="'+ cls +' linedd cltd"><i>'+date1+'<br/>'+date2+'</i></span>' }
                    else{clhtml='<span class="'+ cls +' cltd"><i>'+date1+'<br/>'+date2+'</i></span>'}
      ...
      tablea.append($(cltr));
      table.append(ela);
} $(".cltable").height($(window).height()-175)
  $(window).resize(function(){$(".cltable").height($(window).height()-175)});
  }
...
$(function() {
	...
	getRegionData();checkRegion();getUserData();
	
	$("#doc-ipt-them,#doc-ta-hyms").on('focus', function () {
		  var target = this;
		  setTimeout(function(){
...
if($(this).hasClass("active")){    
    ...
    $('#doc-ipt-date').on("click", function(){ 
    $("#meetingBox").show();
     });
      
    $('#doc-ipt-sb').on("click", function(){ 
        $("#meetingSb").show();getSbData();	
         });  
    
    $('#doc-ipt-user').on("click", function(){ 
        $("#meetingUser").show();getUserData()
         }); 
    ...
    $(document).on("click",".am-checkboxli", function(){
    	...
    	if($(this).hasClass("active")){
    		...
    		$(this).addClass("active");
    		checkboxArrText.push(text);checkboxArr.push(sbId);
    	}
...
$("#doc-ipt-them").focus(function(){
    ...
    $("#doc-ipt-date,#doc-ipt-sb,#doc-ipt-user").click(function(){
    	$(this).removeClass("am-field-error");
    	}); 
...
function getTimeD(){
	 ...
	 var date=$("#date").val();
	 var time = date + ' '+ timestart +'-' + timeend;
	 //console.log("timestart:"+timestart);
	 if(timestart=="undefined" || timestart==''){
		 $("#doc-ipt-date").html("选择你的会议时间");
		 $("#doc-ipt-date").addClass("c9");
		 $("#tpi-checkbox-sb-box").html('');
		 $("#doc-ipt-sb").html('选择你的会议设备');
		 $("#doc-ipt-sb").addClass("c9");
	 }
	 else {
		 $("#doc-ipt-date").html(time);
		 $("#doc-ipt-date").removeClass("c9");
		 getSbData();		 
	 }   
	 $("#meetingBox").hide(); 
	}
function back()	{$(".wrapperbox").hide();}
...
function getUserD(){
	var str = '';
	 ...
	 if(i==arrUser.length-1){ str += arrUser[i];} else {str += arrUser[i] + '、';}
	}
     if(arrUser.length==0){
		 $("#doc-ipt-user").html("选择你的参会人员");
		 $("#doc-ipt-user").addClass("c9");
	 }
	 else {
		 $("#doc-ipt-user").html(str);
		 $("#doc-ipt-user").removeClass("c9");	 
	 }
	 $("#meetingUser").hide();
	 }
...
function checkRoom(roomId){
		...
		$("#doc-ipt-date").html("选择你的会议时间").addClass("c9");
...
function saveData(){	
	...
	if($("#doc-ipt-them").val()==''){$("#doc-ipt-them").addClass("am-field-error");Alert("提示信息","请输入你的会议主题");return   }
	if($("#doc-ipt-date").html()=='' || $("#doc-ipt-date").html()=='选择你的会议时间'){
		$("#doc-ipt-date").addClass("am-field-error");Alert("提示信息","请选择你的会议时间"); return  }
	//if($("#doc-ipt-sb").html()=='' || $("#doc-ipt-sb").html()=='选择你的会议设备'){$("#doc-ipt-sb").addClass("am-field-error");Alert("提示信息","请选择你的会议设备");return  }
	//if($("#doc-ipt-user").html()=='' || $("#doc-ipt-user").html()=='选择你的参会人员'){$("#doc-ipt-user").addClass("am-field-error");Alert("提示信息","请选择你的参会人员");return  }
  	if($("#doc-ta-hyms").val().length > 200){Alert("提示信息","请会议描述控制200字以内"); return }
	$.ajax({  
        type : "POST",  //提交方式  
        url : __ctx+"/me/conference/conferenceInfo/save.ht",
        data : $('#form1').serialize(),
        success : function(result) {
            if ( status==0 ) {  Alert("提示信息",result.message);
        	$("#doc-ipt-them").val('');
        	$("#doc-ta-hyms").val('');
			...
			location.href = "./mRoomList.html?sessionId=" + sessionId;
			
            } else {  
            	Alert("提示信息",JSON.stringify(result.message));
            }  
        }  
    });  
}	
