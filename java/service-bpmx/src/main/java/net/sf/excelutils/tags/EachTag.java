	...
	public int[] parseTag(Object context, HSSFSheet sheet, HSSFRow curRow,
		...
		if (null == it) {
			if (obj instanceof DynaBean) {
				it = ExcelParser.getIterator(ExcelParser
						.getBeanProperties(((DynaBean) obj).getDynaClass()));
			} else {
				it = ExcelParser.getIterator(ExcelParser.getBeanProperties(obj
						.getClass()));
			}
		}
		if (null == it) {
			...
			if (ExcelUtils.isCanShowType(value)) {
				...
				for (int i = 0; i < sheet.getNumMergedRegions(); i++) {
					...
					if (r.getFirstRow() == curRow.getRowNum()
							&& r.getFirstRow() == curCell.getColumnIndex()
							&& r.getLastRow() == curCell.getColumnIndex()) {
				...
				if (width > 1) {
					HSSFCell nextCell = WorkbookUtils.getCell(curRow,
							eachPos + 1);
					WorkbookUtils.shiftCell(sheet, curRow, nextCell, width - 1,
							rowMerged);
				}

				// copy the style of curCell
				for (int rownum = curRow.getRowNum(); rownum < curRow
						.getRowNum() + rowMerged; rownum++) {
					for (int i = 0; i < width; i++) {
						HSSFRow r = WorkbookUtils.getRow(rownum, sheet);
						HSSFCell c = WorkbookUtils.getCell(r, eachPos + i);
						HSSFCell cc = WorkbookUtils.getCell(r,
				...
				if (width > 1 || rowMerged > 1) {
					CellRangeAddress r = new CellRangeAddress(
							curRow.getRowNum(), cell.getColumnIndex(),
							curRow.getRowNum() + rowMerged - 1,
