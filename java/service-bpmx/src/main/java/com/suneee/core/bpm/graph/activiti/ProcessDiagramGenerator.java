...
import com.suneee.core.api.bpm.model.IBpmProStatus;
import com.suneee.core.util.AppUtil;
import com.suneee.core.util.Dom4jUtil;
import com.suneee.core.util.FontUtil;
...
public class ProcessDiagramGenerator {
	...
	public static ProcessDiagramCanvas initProcessDiagramCanvas(List<BPMNShap> shaps,List<BPMNEdge> edges) {
		...
		int minY=(int) Math.round((points[0].getY() -shiftY))-20;
	...
	public static ProcessDiagramCanvas generateDiagram(Map<String,String> highLightedActivities, String bpmnXml) {
		...
		drawSequenceFlowsHigh(processDiagramCanvas, bpmnEdges,highLightedActivities);
		return processDiagramCanvas;
	}
...
//			}
	...
	//走过的流程线变成红色
//	public static void drawSequenceFlowsHigh(ProcessDiagramCanvas processDiagramCanvas, List<BPMNEdge> bpmnEdges,Map<String,String> highLightedActivities) {
//		for (BPMNEdge bpmnEdge : bpmnEdges) {
//			processDiagramCanvas.drawSequenceflowWidthLabelHigh(bpmnEdge,highLightedActivities);
//		}
//	}
	public static void drawSequenceFlowsHigh(ProcessDiagramCanvas processDiagramCanvas, List<BPMNEdge> bpmnEdges,Map<String,String> highLightedActivities) {
		List<BPMNEdge> bpmnEdgelist = new ArrayList<BPMNEdge>();
		//流程同意的map
		Map<String,String> agreeMap = new HashMap<String,String>();
		//还未同意的。比如正在审批的。驳回的。
		Map<String,String> notAgreeMap = new HashMap<String,String>();
		Map<String,String> statusColorMap = (Map<String,String>)AppUtil.getBean("statusColorMap");
		
		for (Map.Entry<String, String> entry : highLightedActivities.entrySet()) {
			//同意和会签通过1/5
			if(entry.getValue().equals(statusColorMap.get("1"))||entry.getValue().equals(statusColorMap.get("5"))){
				agreeMap.put(entry.getKey(), entry.getValue());
			}else{
				notAgreeMap.put(entry.getKey(), entry.getValue());
			}
		}
		for(BPMNEdge bpmnEdge : bpmnEdges){
			if(("StartEvent".equals(bpmnEdge.getSourceRef().substring(0, bpmnEdge.getSourceRef().lastIndexOf('t')+1))&&agreeMap.containsKey(bpmnEdge.getTargetRef()))
					||("EndEvent".equals(bpmnEdge.getTargetRef().substring(0,bpmnEdge.getTargetRef().lastIndexOf('t')+1))&&agreeMap.containsKey(bpmnEdge.getSourceRef()))
					||(agreeMap.containsKey(bpmnEdge.getSourceRef())&&agreeMap.containsKey(bpmnEdge.getTargetRef()))
					||("XORGateway".equals(bpmnEdge.getSourceRef().substring(0,bpmnEdge.getSourceRef().lastIndexOf('y')+1 )))&&agreeMap.containsKey(bpmnEdge.getTargetRef())
					||("XORGateway".equals(bpmnEdge.getTargetRef().substring(0,bpmnEdge.getTargetRef().lastIndexOf('y')+1 )))&&agreeMap.containsKey(bpmnEdge.getSourceRef())
					||("ANDGateway".equals(bpmnEdge.getSourceRef().substring(0,bpmnEdge.getSourceRef().lastIndexOf('y')+1 )))&&agreeMap.containsKey(bpmnEdge.getTargetRef())
					||("ANDGateway".equals(bpmnEdge.getTargetRef().substring(0,bpmnEdge.getTargetRef().lastIndexOf('y')+1 )))&&agreeMap.containsKey(bpmnEdge.getSourceRef())
					||("ORGateway".equals(bpmnEdge.getSourceRef().substring(0,bpmnEdge.getSourceRef().lastIndexOf('y')+1 )))&&agreeMap.containsKey(bpmnEdge.getTargetRef())
					||("ORGateway".equals(bpmnEdge.getTargetRef().substring(0,bpmnEdge.getTargetRef().lastIndexOf('y')+1 )))&&agreeMap.containsKey(bpmnEdge.getSourceRef())
					){
				bpmnEdgelist.add(bpmnEdge);
				//没有通过的出去的线不能标红
			}else if(("StartEvent".equals(bpmnEdge.getSourceRef().substring(0, bpmnEdge.getSourceRef().lastIndexOf('t')+1))&&notAgreeMap.containsKey(bpmnEdge.getTargetRef()))
					||(agreeMap.containsKey(bpmnEdge.getSourceRef())&&notAgreeMap.containsKey(bpmnEdge.getTargetRef()))
					||("XORGateway".equals(bpmnEdge.getSourceRef().substring(0,bpmnEdge.getSourceRef().lastIndexOf('y')+1 )))&&notAgreeMap.containsKey(bpmnEdge.getTargetRef())
					||("ANDGateway".equals(bpmnEdge.getSourceRef().substring(0,bpmnEdge.getSourceRef().lastIndexOf('y')+1 )))&&notAgreeMap.containsKey(bpmnEdge.getTargetRef())
					||("ORGateway".equals(bpmnEdge.getSourceRef().substring(0,bpmnEdge.getSourceRef().lastIndexOf('y')+1 )))&&notAgreeMap.containsKey(bpmnEdge.getTargetRef())
					){
				bpmnEdgelist.add(bpmnEdge);
			}
		}
