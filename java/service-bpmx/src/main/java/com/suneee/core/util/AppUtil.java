...
//	private static Map<Long,OnlineUser> onlineUsers=new LinkedHashMap<Long, OnlineUser>();
	...
	public void initSysService(){
        log.debug("---------[contextInitialized]开始初始化表单模版。");
        //初始化系统模版。
        BpmFormTemplateService.initTemplate();

        log.debug("--------[contextInitialized]初始化表单模版成功。");

        log.debug("[contextInitialized]开始初始化首页栏目。");
        SysIndexColumnService.initIndex();

        log.debug("[contextInitialized]初始化首页栏目成功。");
