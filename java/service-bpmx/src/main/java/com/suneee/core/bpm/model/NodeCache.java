package com.suneee.core.bpm.model;
...
public class NodeCache {
	 ...
	 * @return
	 */
	public static boolean isGateWay(String actDefId,String nodeId){
		getByActDefId(actDefId);
		Map<String, FlowNode> nodeMap = actNodesMap.get(actDefId);
		FlowNode flowNode=nodeMap.get(nodeId);
		if(flowNode.getPreFlowNodes().size()==0) return false;
		//分支返回false
		if(flowNode.getPreFlowNodes().get(0).getNodeId().contains("XORGateway")) return false;
		//判断上一节点是否是同步或条件同步网关
			if(flowNode.getPreFlowNodes().get(0).getNodeId().contains("Gateway")){
				//判断下一节点是否是同步或条件同步网关
				if(flowNode.getNextFlowNodes().get(0).getNodeId().contains("Gateway")) {
					return true;
				}
			}
		return false;
	...
	private static Map<String, FlowNode> parseXml(String xml,FlowNode parentNode) {
		...
		while (its.hasNext()) {
			...
			Node orderNode=nodeEl.selectSingleNode("./extensionElements/ht:order");
			...
			if ("startEvent".equals(nodeType) || FlowNode.TYPE_USERTASK .equals(nodeType)
				...
				FlowNode flowNode = new FlowNode(nodeId, nodeName, nodeType,
						isMultiInstance);
				if (orderNode!=null){
					flowNode.setOrder(Integer.valueOf(orderNode.getText()));
				}
	 ...
	 * @return
	 */
	public static boolean isPreParallelGateway(Map<String,FlowNode> cacheMap,FlowNode node){
		if (node==null){
			return false;
		}
		if ("parallelGateway".equals(node.getNodeType())){
			return true;
		}
		List<FlowNode> nodeList=node.getPreFlowNodes();
		if (nodeList.size()==0){
			return false;
		}
		FlowNode temp=cacheMap.get(node.getNodeId());
		if (temp!=null){
			return false;
		}
		cacheMap.put(node.getNodeId(),node);
		for (FlowNode nodeItem:nodeList){
			if (isPreParallelGateway(cacheMap,nodeItem)){
				return true;
			}
		}
		return false;
	 ...
	 * @return
	 */
	public static boolean isNextParallelGateway(Map<String,FlowNode> cacheMap,FlowNode node){
		if (node==null){
			return false;
		}
		if ("parallelGateway".equals(node.getNodeType())){
		...
		List<FlowNode> nodeList=node.getNextFlowNodes();
		if (nodeList.size()==0){
			return false;
		}
		FlowNode temp=cacheMap.get(node.getNodeId());
		...
		cacheMap.put(node.getNodeId(),node);
		for (FlowNode nodeItem:nodeList){
			if (isNextParallelGateway(cacheMap,nodeItem)){
				return true;
			}
		}
		return false;
	 ...
	 * @return
	 */
	public static boolean isSyncNode(String actDefId,String nodeId){
		FlowNode node=getFlowNode(actDefId,nodeId);
		Map<String,FlowNode> preCacheMap=new HashMap<String, FlowNode>();
		Map<String,FlowNode> nextCacheMap=new HashMap<String, FlowNode>();
		if (isPreParallelGateway(preCacheMap,node)&&isNextParallelGateway(nextCacheMap,node)){
			return true;
		}
		return false;
	}
	 ...
	 * @return
	 */
	public static FlowNode getFlowNode(String actDefId,String nodeId){
		Map<String,FlowNode> flowNodeMap=getByActDefId(actDefId);
		FlowNode node=flowNodeMap.get(nodeId);
		return node;
	}
