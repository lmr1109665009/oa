package com.suneee.core.bpm.util;

import com.suneee.core.api.bpm.IBpmDefinitionService;
import com.suneee.core.api.bpm.model.IBpmDefinition;
import com.suneee.core.bpm.graph.DivShape;
import com.suneee.core.bpm.graph.Point;
import com.suneee.core.bpm.graph.ShapeMeta;
import com.suneee.core.bpm.graph.activiti.BPMNEdge;
import com.suneee.core.bpm.graph.activiti.BPMNShap;
import com.suneee.core.bpm.graph.activiti.ProcessDiagramGenerator;
import com.suneee.core.bpm.model.ForkNode;
import com.suneee.core.bpm.model.NodeCondition;
...
public class BpmUtil {
	...
	public static Map<String, Map<String, String>> getTaskActivitys(String defXml, Boolean flag) {
		...
		Map<String, String> gateWayMap = new LinkedHashMap<String, String>();
		// 并行网关
		...
		Map<String, String> endMap = new LinkedHashMap<String, String>();
	...
	private static void addToMap(List<Node> list, Map<String, String> map) {
		...
		Collections.sort(nodeList, new Comparator<Element>() {
			...
			public int compare(Element o1, Element o2) {
				Node order1=o1.selectSingleNode("./extensionElements/ht:order");
				Node order2=o2.selectSingleNode("./extensionElements/ht:order");
				if (order1==null||order2==null){
					return 0;
				}
				int sort1=Integer.valueOf(order1.getText());
				int sort2=Integer.valueOf(order2.getText());
				return sort1-sort2;
			}
		});
	 ...
	 * 
	 * @param className
	 *            类名 如 com.suneee.platform.service.bpm.listener.StartEventListener
	 * @return
	 */
...
//				el.add(conditionEl);
	...
	public static ProcessCmd getProcessCmd(HttpServletRequest request) throws Exception {
        ...
        temp = request.getParameter("system");
        if (StringUtil.isNotEmpty(temp)) {
            cmd.setSystem(temp);
        }
		...
		cmd.setInformType(request.getParameter("informType"));
...
//		}
		...
		String[] destTaskIds = RequestUtil.getStringAryByStr(request, "lastDestTaskId");
		if (destTaskIds != null) {
		 ...
		 * 退回到分发任务的对象
		 */
		temp = request.getParameter("forkTaskRejects");
		if (StringUtil.isNotEmpty(temp)) {
			List<ForkTaskReject> forkTaskRejects = ForkTaskReject.parser2List(temp);
			if(BeanUtils.isNotEmpty(forkTaskRejects)){
				ForkTaskReject forkTaskReject = forkTaskRejects.get(0);
				//设置退回到分发节点
				cmd.setDestTask(forkTaskReject.getNodeId());
			}
			cmd.setForkTaskRejects(forkTaskRejects);
		}
				...
				if (BeanUtils.isNotEmpty(obj)) {
					try {
						...
						if (obj instanceof Date) {
							Date date = (Date) obj;
							if ("00:00:00".equals(DateFormatUtil.format(date, "HH:mm:ss"))) {
								value = DateFormatUtil.formatDate(date);
							} else {
								value = DateFormatUtil.formaDatetTime((Date) obj);
							}
						} else {
							value = obj.toString();
						}
	...
	public static ProcessCmd getProcessCmds(HttpServletRequest request, String taskId) {
