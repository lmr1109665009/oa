package com.suneee.core.web.query;
...
public class QueryFilter {
	...
	public QueryFilter(HttpServletRequest request, Boolean isLg, String tableId) {
		...
		try {
			...
			if (StringUtil.isNotEmpty(orderField)) {
				orderField = orderField.equals("id") ? orderField : TableModel.CUSTOMER_COLUMN_PREFIX + orderField;
	 ...
	 * 传入一个JSON如果json包含currentPage 则表示需要分页，否则不分页。
	 * {
	 * 	pageSize:1,
	 * 	currentPage:2,
	 * 	sortField:,
	 ...
	 * SLL:字符串 左模糊查询
	 * SLR:字符串 右模糊查询
	 * @param json	:查询信息，包含分页信息，排序信息，Q_参数名称_参数类型 
	 */
	public QueryFilter(JSONObject json) {
		
		try {
			//获取分页信息
			String currentPageStr = json.get("currentPage") != null ? json.getString("currentPage") : "";
			String pageSizeStr = json.get("pageSize") != null ? json.getString("pageSize") : "";
			Integer pageSize = StringUtil.isNotEmpty(pageSizeStr) ? Integer.parseInt(pageSizeStr) : PageBean.DEFAULT_PAGE_SIZE;//分页大小
			Integer currentPage = StringUtil.isNotEmpty(currentPageStr) ? Integer.parseInt(currentPageStr) : 1;//获取当前页
			if(StringUtil.isNotEmpty(currentPageStr)){
				this.pageBean = new PageBean(currentPage, pageSize);
			} 
			//处理其他查询信息
			Map<String, Object> map = JSONObjectUtil.getQueryMap(json);
			this.filters = map;
		} catch (Exception ex) {
			ex.printStackTrace();
			logger.error(ex.getMessage());
