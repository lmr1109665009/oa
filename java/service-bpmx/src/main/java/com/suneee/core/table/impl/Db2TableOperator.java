package com.suneee.core.table.impl;
...
import com.suneee.core.page.PageBean;
import com.suneee.core.table.AbstractTableOperator;
...
//		}
	...
	public TableIndex getIndex(String tableName, String indexName) {
		...
		List<TableIndex> indexes = jdbcTemplate.query(sql, indexRowMapper);
		List<TableIndex> indexList = mergeIndex(indexes);
		if(BeanUtils.isEmpty(indexList)){
			return null;
		}else{
			TableIndex index = indexList.get(0);
	 ...
	 * @see com.suneee.core.customertable.AbstractTableOperator#rebuildIndex(String, String)
	 */
	@Override
	...
	private List<TableIndex> mergeIndex(List<TableIndex> indexes){
		...
		for (TableIndex index : indexes) {
			boolean found=false;
			for (TableIndex index1 : indexList) {
	...
	private String generateIndexDDL(TableIndex index){
		...
		if(!StringUtil.isEmpty(index.getIndexType())){
			if(TableIndex.INDEX_TYPE_CLUSTERED.equalsIgnoreCase(index.getIndexType())){
				sql.append(" CLUSTER ");
			}
		}
	...
	private RowMapper<TableIndex> indexRowMapper=new RowMapper<TableIndex>() {
		@Override
		public TableIndex mapRow(ResultSet rs, int rowNum) throws SQLException {
			TableIndex index=new TableIndex();
			index.setIndexTable(rs.getString("TABNAME"));
			index.setTableType(TableIndex.TABLE_TYPE_TABLE);
			...
			if("CLUS".equalsIgnoreCase(indexType)){
				index.setIndexType(TableIndex.INDEX_TYPE_CLUSTERED);
			}else if("REG".equalsIgnoreCase(indexType)){
				index.setIndexType(TableIndex.INDEX_TYPE_REG);
			}else if("DIM".equalsIgnoreCase(indexType)){
				index.setIndexType(TableIndex.INDEX_TYPE_DIM);
			}else if("BLOK".equalsIgnoreCase(indexType)){
				index.setIndexType(TableIndex.INDEX_TYPE_BLOK);
			}
