...
import com.suneee.core.ldap.dao.LdapUserDao;
import com.suneee.core.ldap.map.LdapUserAssembler;
...
public class LdapUserDaoImpl implements LdapUserDao {
	 ...
	 * 通过分页查询所有的用户
	 * (non-Javadoc)
	 * @see com.suneee.core.ldap.dao.LdapUserDao#get()
	 */
	...
	public List<LdapUser> get(){
		...
		List<LdapUser> users = (List<LdapUser>) pagedResult.getResultList();
		...
		while(users.size()==pageSize && bytesCookie!=null){
			...
			users= (List<LdapUser>) pagedResult.getResultList();
			ldapUserList.addAll(users);
	 ...
	 * 通过分页，查询用户
	 *  (non-Javadoc)
	 * @see com.suneee.core.ldap.dao.LdapUserDao#get(org.springframework.ldap.control.PagedResultsCookie, int)
	 */
	@SuppressWarnings("unchecked")
	@Override
