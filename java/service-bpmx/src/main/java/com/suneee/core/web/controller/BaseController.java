...
import com.suneee.core.util.ConfigUtil;
...
public class BaseController extends GenericController
{
  ...
  /**
	...
	protected Map<String, Object> getMapByPageListJq(PageList pageList){
		Map<String,Object> map=new HashMap<String, Object>();
		map.put("rows", pageList);
		map.put("records", pageList.getTotalCount());
		map.put("page", pageList.getPageBean().getCurrentPage());
	...
	protected String getServerUrl(HttpServletRequest request){
		...
		if (StringUtil.isEmpty(hostname)){
			hostname=request.getServerName()+":"+request.getServerPort();
		}
		return request.getScheme()+"://"+hostname+path;
	}
