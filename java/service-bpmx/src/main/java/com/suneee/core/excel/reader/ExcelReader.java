package com.suneee.core.excel.reader;
...
public class ExcelReader {
	...
	private List<FieldEntity> getLine(HSSFSheet sheet, int row) {
		...
		// 获取标题头
		String[] titleColumns = this.excelReaderConfig.getColumns();
		// 获到当前行的列数
		int filledColumns = rowline.getLastCellNum();
		// 如果当前行的列数大于标题头的列数，则便利处理的列数取标题头的列数
		if(filledColumns > titleColumns.length){
			filledColumns = titleColumns.length;
		}
		HSSFCell cell = null;
		// 开始读取的列，从第几列开始读。
		...
		for (int i = colStart; i < filledColumns; i++) {
			...
			String column = titleColumns[i-colStart];			
