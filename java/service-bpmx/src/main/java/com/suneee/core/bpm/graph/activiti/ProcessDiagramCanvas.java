...
public class ProcessDiagramCanvas {
	...
	//走过的流程线变成红色
	public void drawSequenceflowHigh(List<Point2D.Double> points,boolean highlight) {
		for (int i = 0; i < points.size() - 1; i++) {
			Line2D.Double line = new Line2D.Double(points.get(i).getX(), points
					.get(i).getY(), points.get(i + 1).getX(), points.get(i + 1)
					.getY());
	...
	//走过的流程线变成红色
		public void drawSequenceflowWidthLabelHigh(BPMNEdge bpmnEdge,Map<String,String> highLightedActivities) {
		//都过的流程线设为true。提示drawSequenceflowHigh方法要用红色标记
		drawSequenceflowHigh(bpmnEdge.getPoints(),true);
		int flag=0;
		DirectionType directionType=bpmnEdge.getDirection();
		if(directionType==DirectionType.UpToDown){
			flag=1;
	...
	public void drawSequenceflowWidthLabelHigh02(BPMNEdge bpmnEdge,Map<String,String> highLightedActivities) {
		boolean highlight = false;
		List<BPMNEdge> bpmnEdgelist = null;
		if(("StartEvent1".equals(bpmnEdge.getSourceRef())&&highLightedActivities.containsKey(bpmnEdge.getTargetRef()))||("EndEvent1".equals(bpmnEdge.getTargetRef())&&highLightedActivities.containsKey(bpmnEdge.getSourceRef()))||(highLightedActivities.containsKey(bpmnEdge.getSourceRef())&&highLightedActivities.containsKey(bpmnEdge.getTargetRef()))){
			highlight = true;
		}
		drawSequenceflowHigh(bpmnEdge.getPoints(),highlight);
		int flag=0;
		DirectionType directionType=bpmnEdge.getDirection();
		if(directionType==DirectionType.UpToDown){
			flag=1;
