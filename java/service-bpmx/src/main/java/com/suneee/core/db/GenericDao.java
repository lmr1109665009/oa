...
import com.suneee.core.mybatis.BaseMyBatisDao;
import com.suneee.core.mybatis.IbatisSql;
 ...
 * 2.<a name="完整路径">完整路径</a>,如com.suneee.platform.model.Role.getRole。<br>
 * @version 1.0
 */
...
public abstract class GenericDao<E, PK extends Serializable> extends BaseMyBatisDao implements IEntityDao<E, PK>
{
	...
	public void add(E entity)
	{
		...
		if(entity instanceof BaseModel)
		{
			...
			if(baseModel.getCreateBy() == null){
				baseModel.setCreateBy(ContextSupportUtil.getCurrentUserId());
			}
			if(baseModel.getUpdatetime() == null){
				baseModel.setUpdatetime(new Date());
			}
			if(baseModel.getUpdateBy() == null){
				baseModel.setUpdateBy(ContextUtil.getCurrentUserId());
			}
			//通过接口调用
			try{
				IDbSetModel iDbSetModel= AppUtil.getBean(IDbSetModel.class);
				if (baseModel.getCreateBy() == null ){
					if(iDbSetModel!=null){
						iDbSetModel.setAdd(baseModel);
					}
				}
			}
			catch(NoSuchBeanDefinitionException ex){
				logger.debug("add:NoSuchBeanDefinitionException" + ex.getMessage());
			}
	...
	public int update(E entity)
	{
		...
		if(entity instanceof BaseModel)
		{
			...
			try{
				IDbSetModel iDbSetModel=AppUtil.getBean(IDbSetModel.class);
				if(iDbSetModel!=null){
					iDbSetModel.setUpd(baseModel);
				}
			}
			catch(NoSuchBeanDefinitionException ex){
				logger.debug("UPDATE:NoSuchBeanDefinitionException" + ex.getMessage());
			}
	 ...
	 * 返回如：com.suneee.platform.model.role.Role
	 * @return
	 */
	...
	public List<E> getList(String statementName,Object params,PageBean pageBean)
	{
		...
		if(pageBean.isShowTotal()){
			...
			//取得返回数量
			int totalCount=jdbcTemplate.queryForObject(ibatisSql.getCountSql(),Integer.class,ibatisSql.getParameters());
	...
	public List<E> getList(String statementName,Object params)
	{
		...
		return getSqlSessionTemplate().selectList(statementName, filters);
	}
	 ...
	 * @return
	 */
	public List<E> getAllCountByCate(QueryFilter filter)
	{
		String statementName = getIbatisMapperNamespace() + ".getAllCountByCate";
		List<E> list= getList(statementName,filter);
		//设置回请求对象
		filter.setForWeb();
		return list;
	}
	...
	public E getEBySqlKey(String sqlKey,Object params)
	{
		String statement= getIbatisMapperNamespace() + "." + sqlKey;
