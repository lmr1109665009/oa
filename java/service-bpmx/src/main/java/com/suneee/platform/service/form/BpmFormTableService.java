...
import com.suneee.platform.dao.form.*;
...
import com.suneee.platform.xml.table.BpmFormTableXmlList;
...
//		table.setCreator(sysUser.getFullname());
		table.setCreator(ContextSupportUtil.getUsername(sysUser));
	...
	public int upd(BpmFormTable table, int generate) throws Exception {
		...
		if (hasFormDef || hasData) {
			...
			for (BpmFormField field : updList) {
				...
				updateFormTableField(field,tableName,originFieldList);
			}
			// 没有增加的列就直接返回。
...
//					table.setPublishedBy(sysUser.getFullname());
					table.setPublishedBy(ContextSupportUtil.getUsername(sysUser));
				}
				// 直接该表的定义。
				dao.update(table);
				// 删除字段，重新加入字段
	 ...
	 * @throws SQLException
	 */
	private void updateFormTableField(BpmFormField field, String tableName, List<BpmFormField> originFieldList) throws Exception {
		boolean isFind=false;
		BpmFormField originField=null;
		for (BpmFormField formField:originFieldList){
			if (formField.getFieldName().equals(field.getFieldName())){
				isFind=true;
				originField=formField;
				break;
			}
		}
		if (!isFind){
		...
		//如果两者生成的唯一标识相同，则不需要更新数据库表字段
		if (originField.generateUniqueCode().equals(field.generateUniqueCode())){
			return;
		}
		ColumnModel columnModel = getByField(field, 2);
		tableOperator.updateColumn(TableModel.CUSTOMER_TABLE_PREFIX + tableName,columnModel.getName(),columnModel);
	}
	 ...
	 * @param table
	 */
	private void isSubTableGenerate(BpmFormTable table) {
		if (table.getIsMain()==BpmFormTable.IS_MAIN){
			return;
		}
		Long mainTableId=table.getMainTableId();
		BpmFormTable mainTable=getById(mainTableId);
		if (mainTable.getIsPublished()==BpmFormTable.NOT_PUBLISH){
			throw new RuntimeException("请先点击生成主表，再点击生成子表");
		}
	}
	...
	public List<BpmFormTable> getAllUnpublishedMainTable() {
		...
		List<Long> typeIdList=globalTypeDao.getTypeIdsByEcode(ecode);
		if (typeIdList.size()==0){
			typeIdList.add(-1L);
		}
		filter.addFilter("typeIdList",typeIdList);
		List<BpmFormTable> list = dao.getAllUnpublishedMainTable(filter);
		return list;
	}
	 ...
	 * @throws Exception
	 */
	public void saveCopy(Long tableId) throws Exception {
		Map<Long, Long> mainTable = new HashMap<Long, Long>();
		if (BeanUtils.isEmpty(tableId))
			throw new RuntimeException("表ID不能为空！");
		BpmFormTable bpmFormTable = dao.getById(new Long(tableId));
		Map<String,Object> tableInfo=getCopyTableInfo(bpmFormTable,1);
	 ...
	 * @return
	 */
	public Map<String,Object> getCopyTableInfo(BpmFormTable formTable,int pos){
		String suffix="_copy\\d+$";
		String tableName=formTable.getTableName();
		if (Pattern.matches(suffix,tableName)){
			tableName=tableName.replaceFirst(suffix,tableName);
		}
		String newTableName=tableName+"_copy"+pos;
		if (isTableNameExisted(newTableName)){
			return getCopyTableInfo(formTable,pos++);
		}
		Map<String,Object> info=new HashMap<String, Object>();
		info.put("tableName",newTableName);
		String tableDesc=formTable.getTableDesc()+"_复制"+pos;
	...
	private BpmFormTable importFormTable(BpmFormTable bpmFormTable) throws Exception {
		...
		if (BeanUtils.isEmpty(table)) {
			...
			bpmFormTable.setTableId(UniqueIdUtil.genId());
			if (bpmFormTable.getIsMain().shortValue() == BpmFormTable.IS_MAIN){
			BpmImportHolder.getTableIdHolder().set(bpmFormTable.getTableId());
			}else if (BpmImportHolder.getTableIdHolder().get()!=null){
				bpmFormTable.setMainTableId(BpmImportHolder.getTableIdHolder().get());
			}
			...
			table.setTableId(bpmFormTable.getTableId());
			SysUser user= (SysUser) ContextUtil.getCurrentUser();
			table.setCreateBy(user.getUserId());
//			table.setCreator(user.getFullname());
			table.setCreator(ContextSupportUtil.getUsername(user));

			//设置分类
			GlobalTypeHolder holder=BpmImportHolder.getTableDefaultTypeHolder().get();
			GlobalTypeService typeService= holder.getTypeService();
			GlobalType category=null;
			if (table.getCategoryId()!=null){
				category=typeService.getById(table.getCategoryId());
			}
			if (category==null){
			...
			table.setCategoryId(category.getTypeId());
			table.setCategoryName(category.getTypeName());
