...
import com.suneee.core.api.bpm.model.ITaskOpinion;
...
import com.suneee.platform.service.bpm.TaskForkService;
...
public class TaskCreateListener extends BaseTaskListener {
	...
	private void handAssignUserFromDb(String actDefId,String nodeId,DelegateTask delegateTask){
		...
		if(processInstance!=null){
			...
			if(vars.get(BpmConst.StartUser) != null){
				startUserId = String.valueOf(vars.get(BpmConst.StartUser));
			}
			...
			//弃权情况下获取上一个节点审批人
			users=getLastNodeExecutor(delegateTask);
	 ...
	 * @return
	 */
	private List<TaskExecutor> getLastNodeExecutor(DelegateTask delegateTask) {
		ExecutionStack executionStack = executionStackDao.getLastestStack(delegateTask.getProcessInstanceId(), delegateTask.getTaskDefinitionKey());
		if (executionStack==null){
			return null;
		}
		List<TaskExecutor> taskExecutorList=new ArrayList<>();
		String[] executorIds=executionStack.getAssignees().split(",");
		for (String executorId:executorIds){
			SysUser user=sysUserDao.getById(Long.valueOf(executorId));
			if (user==null){
				continue;
			}
			taskExecutorList.add(new TaskExecutor(TaskExecutor.USER_TYPE_USER,executorId,ContextSupportUtil.getUsername(user)));
		}
			...
			if(BeanUtils.isEmpty(taskExecutors)){
				...
				//否则，从数据库获取人员设置
					...
					if(processInstance!=null){
						...
						//vars.putAll(processCmd.getVariables());
						String preTaskUser=ContextUtil.getCurrentUserId().toString();
						String actInstId=delegateTask.getProcessInstanceId();
						Object startUser=delegateTask.getVariable(BpmConst.StartUser);
						String startUserId=startUser==null?null:String.valueOf(startUser);
	...
	private void assignUser(DelegateTask delegateTask, TaskExecutor taskExecutor){
		if(TaskExecutor.USER_TYPE_USER.equals(taskExecutor.getType())){
			...
			taskOpinion.setExeFullname(ContextSupportUtil.getUsername(exeUser));
	...
	private void assignUser(DelegateTask delegateTask, List<TaskExecutor> executors){
		if(BeanUtils.isEmpty(executors)){
			//如果没有执行人，则获取流程定义设置，判断是否设置执行人为空跳过
			ProcessCmd cmd = TaskThreadService.getProcessCmd();
			BpmDefinition bpmDefinition = bpmDefinitionDao.getByActDefId(cmd.getActDefId());
			String skipSetting = bpmDefinition.getSkipSetting();
			if(skipSetting.indexOf("isEmptyUser")!=-1){
				//如果设置了执行人为空跳过。这个默认当前人为执行人。并记录流程变量标识
				executors = BpmUtil.getTaskExecutors(String.valueOf(ContextUtil.getCurrentUserId()));
				Map<String, Object> var = cmd.getVariables();
				//系统原有核心代码，解决同步里某个节点选不到人时会在共用对象processCmd.variable添加taskUserIsNull标识，会影响其他同步节点判断
				//var.put("taskUserIsNull","Y");
				//修改后代码，增加节点ID作为字段前缀，避免各个节点使用同一个变量进行判断
				var.put(delegateTask.getTaskDefinitionKey()+"_taskUserIsNull","Y");
				cmd.setVariables(var);
				TaskThreadService.setProcessCmd(cmd);
			}else{
