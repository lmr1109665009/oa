...
import com.suneee.platform.webservice.api.UserOrgService;
...
import com.suneee.platform.service.system.SysOrgParamService;
import com.suneee.platform.service.system.SysOrgRoleService;
import com.suneee.platform.service.system.SysOrgService;
import com.suneee.platform.service.system.SysOrgTacticService;
...
public class UserOrgServiceImpl implements UserOrgService {
	...
	@Resource
	SysOrgTacticService sysOrgTacticService;
	...
	public String removeUserPosRel(String userAccount, String posCode) {
		try {
			...
			userPositionService.delByUserIdAndPosId(sysUser.getUserId(),position.getPosId(), UserPosition.DELFROM_USER_POS_DEL);
			return getReturn(true);
		} catch (Exception e) {
			return getReturn(false, ExceptionUtils.getRootCauseMessage(e));
		}
	...
	@Override
	public String getOrg(String json) {
		JSONObject jsonObject = JSONObject.fromObject(json);
		String action = jsonObject.getString("action");

		if ("getByOrgId".equals(action)) {
			Long orgId = jsonObject.getLong("orgId");
			SysOrg sysOrg = sysOrgService.getById(orgId);
			return JSONObject.fromObject(sysOrg).toString();
		}
		return null;
	}

	@Override
	public String getPos(String json) {
		JSONObject jsonObject = JSONObject.fromObject(json);

		String action = jsonObject.getString("action");

		if (action.equals("getByPosId")) {
			Long posId = jsonObject.getLong("posId");
			Position position = positionService.getById(posId);
			if (position != null)
				return JSONObject.fromObject(position).toString();
		}

		if (action.equals("getDefaultPosByUserId")) {
	...
	@Override
	public String getSysOrgListByOrgTactic() {
		List<SysOrg> orgs = sysOrgTacticService.getSysOrgListByOrgTactic();
		JsonArray jarray = this.getOrgJsonArray(orgs);
		if(null != jarray){
	...
	public String getAllRole() {
		...
		for (SysRole sysRole : list) {
			...
			jobject.addProperty("alias", alias);
			jobject.addProperty("roleId", roleId);
			jobject.addProperty("roleName", roleName);
			jarray.add(jobject);
		}
		return jarray.toString();
	}
	...
	@Override
	public String getUserByOrg(String orgCode) {
		try{
			if(StringUtil.isEmpty(orgCode)){
				throw new Exception("必须传入组织代码(orgCode)");
			}
			SysOrg org = sysOrgService.getByCode(orgCode);
			if(BeanUtils.isEmpty(org)){
				throw new Exception("该代码的组织不存在");
			...
			for (SysUser user : users) {
				...
				jobject.addProperty("fullname", user.getFullname());
				jobject.addProperty("aliasName", user.getAliasName());
				jobject.addProperty("account", user.getAccount());
				jobject.addProperty("email", user.getEmail());
				jobject.addProperty("mobile", user.getMobile());
				jobject.addProperty("phone", user.getPhone());
				jobject.addProperty("sex", user.getSex());
				if (user.getEntryDate() != null) {
					jobject.addProperty("entryDate", DateFormatUtil.formatDate(user.getEntryDate()));
				}
				if (user.getWorkDate() != null) {
					jobject.addProperty("workDate", DateFormatUtil.formatDate(user.getWorkDate()));
				}
				List<UserPosition> userPos = userPositionService.getByUserId(user.getUserId());
				if(userPos == null || userPos.isEmpty()){
					jobject.addProperty("posName", "");
				}else{
					jobject.addProperty("posName", userPos.get(0).getPosName());
				}
				
				jobject.addProperty("staffNo", user.getStaffNo());
				jobject.addProperty("identification", user.getIdentification());
				jobject.addProperty("attendNo", user.getAttendNo());
				jarray.add(jobject);
			}
			return jarray.toString();
		}
	...
	@Override
	public String getOrgByOrgSupId(Long orgId) {
		if(orgId == null || orgId == 0){
			orgId = 1L;
		}
		List<SysOrg> sysOrgList = sysOrgService.getOrgByOrgSupId(orgId);
		JsonArray jarray = this.getOrgJsonArray(sysOrgList);
		if(null != jarray){
			return jarray.toString();
		}
		return null;
	...
	private JsonArray getOrgJsonArray(List<SysOrg> sysOrgList){
		...
		for (SysOrg org : sysOrgList) {
			JsonObject jobject = new JsonObject();
			jobject.addProperty("orgId", org.getOrgId());
			jobject.addProperty("demId", org.getDemId());
			jobject.addProperty("orgSupId", org.getOrgSupId());
			jobject.addProperty("code", org.getCode());
			jobject.addProperty("orgName", org.getOrgName());
			jobject.addProperty("orgDesc", org.getOrgDesc());
			jarray.add(jobject);
		}
		return jarray;
	}
