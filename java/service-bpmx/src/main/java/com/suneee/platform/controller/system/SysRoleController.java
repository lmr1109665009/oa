...
public class SysRoleController extends BaseController {
	...
	public ModelAndView selector(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		List<String> categoryList = sysRoleService.getDistinctCategory(new QueryFilter());
	...
	public ModelAndView list(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		for (SysRole role: list) {
			if(role.getAllowDel()==0||role.getAllowEdit()==0){
				if(isAdmin){
					role.setAllowEdit((short)1);
					role.setAllowDel((short)1);
				}
			}
		}
		List<String> categoryList = sysRoleService.getDistinctCategory(new QueryFilter());
