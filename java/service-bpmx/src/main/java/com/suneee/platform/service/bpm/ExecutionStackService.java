...
import com.suneee.platform.dao.bpm.ExecutionStackDao;
...
//				parentStack = dao.getById(executionStack.getParentId());
				parentStack = getByOpinion(executionStack, taskToken);
	...
	/**
	 * 当回退或退回任务时，需要产生堆栈树节点上的兄弟节点对应的任务
	 * @param parentStack
	 * @param copyTaskId
	 */
	...
	public ExecutionStack getByOpinion(ExecutionStack parentStack,String taskToken){
		ExecutionStack parentStackNode = dao.getById(parentStack.getParentId());
		Long actInstId = parentStackNode.getActInstId();
		String taskKey = parentStackNode.getNodeId();
		
		List<TaskOpinion> opinion = taskOpinionService.getByActInstIdTaskKey(actInstId, taskKey);
		while(opinion.size()==0){
			parentStackNode = dao.getById(parentStackNode.getParentId());
			actInstId = parentStackNode.getActInstId();
			taskKey = parentStackNode.getNodeId();
			opinion = taskOpinionService.getByActInstIdTaskKey(actInstId, taskKey);
		}
