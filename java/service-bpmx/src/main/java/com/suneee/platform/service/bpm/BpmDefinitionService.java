...
//		}
		...
		List<BpmFormRights> defRight=bpmFormRightsDao.getByActDefId(oldActDefId, null);
		if(BeanUtils.isNotEmpty(defRight)){
			for(BpmFormRights o:defRight){
				BpmFormRights n=(BpmFormRights)o.clone();
				n.setId(UniqueIdUtil.genId());
//				n.setDefKey(defKey);
				n.setActDefId(newActDefId);
				bpmFormRightsDao.add(n);
			}
		}
...
//			TaskReminder taskReminder = null;
	...
	public void importZip(MultipartFile fileLoad) throws Exception {
		String realFilePath = ServiceUtil.getBasePath()
				+ File.separator
				+ "attachFiles"
				+ File.separator
				+ "tempUnZip"
		...
		if (StringUtils.isEmpty(xmlStr)){
			...
			throw new BusDataException("导入内容未按指定的格式");
		}
	...
	private void importBpmDefinitionXml(BpmDefinitionXml bpmDefinitionXml,
		...
		if (BeanUtils.isNotEmpty(bpmFormTableXmlList)) {
			...
			for (BpmFormTableXml bpmFormTableXml : bpmFormTableXmlList) {
				// 导入表，并解析相关信息
				Map<Long,Long> tableIdTempMap=bpmFormTableService.importBpmFormTableXml(bpmFormTableXml);
				bpmFormTableService.setIdentity(
						bpmFormTableXml.getIdentityList(), identitySet);
				for (Long tableId:tableIdTempMap.keySet()){
					tableIdMap.put(tableId,tableIdTempMap.get(tableId));
				}
			}
		...
		if (BeanUtils.isNotEmpty(bpmFormDefXmlList))
			for (BpmFormDefXml bpmFormDefXml : bpmFormDefXmlList) {
				Map<String, Map> mapForm =null;
				if (identitySet!=null){
					mapForm=bpmFormDefService.importBpmFormDef(bpmFormDefXml,identitySet,tableIdMap);
				}else {
					mapForm=bpmFormDefService.importBpmFormDef(bpmFormDefXml);
				}
...
//		
	 ...
	 * @return
	 */
	public List<BpmDefinition> getByMobileFormKey(String mobileFormKey){
		return dao.getByMobileFormKey(mobileFormKey);
	}
