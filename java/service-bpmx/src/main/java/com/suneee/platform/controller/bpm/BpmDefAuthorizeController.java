...
public class BpmDefAuthorizeController extends BaseController
{
	...
	public void save(HttpServletRequest request,
		...
		if(id>0){
			...
			bpmDefAuthorize.setEnterpriseCode(ContextSupportUtil.getCurrentEnterpriseCode());
		}
...
/*		if(BPMDEFAUTHORIZE_RIGHT_TYPE.MANAGEMENT.equals(authorizeType)){
		...
		if (bpmDefAuthorizeService.checkNameExist(bpmDefAuthorize)){
			HttpUtil.writeResponseJson(response,ResponseMessage.fail("授权名称已存在，请不要重复添加！"));
			return;
		}
		...
		if(myId>0){
			HttpUtil.writeResponseJson(response,ResponseMessage.success("保存授权信息成功！"));
		}else{
			HttpUtil.writeResponseJson(response,ResponseMessage.fail("保存授权信息失败！"));
		}
	}
