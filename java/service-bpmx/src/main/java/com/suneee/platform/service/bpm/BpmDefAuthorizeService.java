...
import com.suneee.platform.dao.system.SysRoleDao;
...
public class BpmDefAuthorizeService extends BaseService<BpmDefAuthorize>
{
	...
	public String toOwnerNameJson( List<BpmDefUser> myBpmDefUserList){
		...
		if(myBpmDefUserList==null||myBpmDefUserList.size()<1){
			...
			for (BpmDefUser bpmDefUser : myBpmDefUserList){
				...
				if(BpmDefUser.BPMDEFUSER_RIGHT_TYPE.ALL.equals(rightType)){
					allList.add(bpmDefUser);
					break;
				}else if(BpmDefUser.BPMDEFUSER_RIGHT_TYPE.USER.equals(rightType)){
					userList.add(bpmDefUser);
				}else if(BpmDefUser.BPMDEFUSER_RIGHT_TYPE.ROLE.equals(rightType)){
					roleList.add(bpmDefUser);
				}else if(BpmDefUser.BPMDEFUSER_RIGHT_TYPE.ORG.equals(rightType)){
					orgList.add(bpmDefUser);
				}else if(BpmDefUser.BPMDEFUSER_RIGHT_TYPE.GRANT.equals(rightType)){
					grantList.add(bpmDefUser);
				}else if(BpmDefUser.BPMDEFUSER_RIGHT_TYPE.POSITION.equals(rightType)){
		...
		if(obj.containsKey("objJson")){
			...
			if(objJson.containsKey("allJson")){
				...
				if("Y".equals(allJson)){
					...
					objUser.setOwnerName(BpmDefUser.BPMDEFUSER_RIGHT_TYPE.ALL);
					objUser.setRightType(BpmDefUser.BPMDEFUSER_RIGHT_TYPE.ALL);
	...
	public List<BpmDefUser> getJsontoList(JSONObject objJson, List<BpmDefUser> myBpmDefUserList, String jsonName, Long authorizeId){
		if(objJson.containsKey(jsonName)){
			...
			if("orgJson".equals(jsonName)){
				rightType = BpmDefUser.BPMDEFUSER_RIGHT_TYPE.ORG;
			}else if("roleJson".equals(jsonName)){
				rightType = BpmDefUser.BPMDEFUSER_RIGHT_TYPE.ROLE;
			}else if("grantJson".equals(jsonName)){
				rightType = BpmDefUser.BPMDEFUSER_RIGHT_TYPE.GRANT;
			}else if("positionJson".equals(jsonName)){
				rightType = BpmDefUser.BPMDEFUSER_RIGHT_TYPE.POSITION;
			}else if("userJson".equals(jsonName)){
				rightType = BpmDefUser.BPMDEFUSER_RIGHT_TYPE.USER;
			}
			//没有类型的直接返回
			if("".equals(rightType)){
				return myBpmDefUserList;
	...
	public Map<String,Object> getActRightByUserMap(Long userId,String authorizeType,boolean isRight,boolean isMyDef){
		...
		Map<String,Object> map = new HashMap<String, Object>();
		map.put("enterpriseCode", ContextSupportUtil.getCurrentEnterpriseCode());
		map.put("authorizeType", authorizeType);
		String authorizeIds = "";
		//要查询的类型类型
		map.put("all", BpmDefUser.BPMDEFUSER_RIGHT_TYPE.ALL);
		map.put("user", BpmDefUser.BPMDEFUSER_RIGHT_TYPE.USER);
		map.put("role", BpmDefUser.BPMDEFUSER_RIGHT_TYPE.ROLE);
		map.put("org", BpmDefUser.BPMDEFUSER_RIGHT_TYPE.ORG);
		map.put("position", BpmDefUser.BPMDEFUSER_RIGHT_TYPE.POSITION);
		map.put("grant", BpmDefUser.BPMDEFUSER_RIGHT_TYPE.GRANT);
		...
		try {
			roleIdList = this.getRoleIdsByUserId(userId, ContextSupportUtil.getCurrentEnterpriseCode());
		...
		if(isMyDef){
			...
			if(myList.size()>0){
				...
				if(isRight){
					...
					authorizeRight.setRightByAuthorizeType("Y", BpmDefAuthorizeType.BPMDEFAUTHORIZE_RIGHT_TYPE.MANAGEMENT);
	 ...
	 * @return
	 */
	public boolean checkNameExist(BpmDefAuthorize authorize) {
		QueryFilter filter=new QueryFilter();
		filter.addFilter("name",authorize.getAuthorizeDesc());
		filter.addFilter("enterpriseCode",ContextSupportUtil.getCurrentEnterpriseCode());
		if (authorize.getId()!=null&&authorize.getId()>0){
			filter.addFilter("id",authorize.getId());
		}
		List<BpmDefAuthorize> authorizeList=bpmDefAuthorizeDao.getByName(filter);
		return authorizeList.size()>0;
	}
	 ...
	 */
	public List<Long> getRoleIdsByUserId(Long userId,String enterpriseCode) throws UnsupportedEncodingException{
		MultiValueMap<String,Object> params=new LinkedMultiValueMap<>();
		params.add("userId",userId);
		params.add("enterpriseCode",enterpriseCode);
		ResponseEntity<List<Long>> responseEntity= (ResponseEntity<List<Long>>) RestTemplateUtil.get(ServiceConstant.getOaServiceUrl() + RoleResourceApi.getRoleIdsByUserId,
				new ParameterizedTypeReference<List<Long>>() {},params);
