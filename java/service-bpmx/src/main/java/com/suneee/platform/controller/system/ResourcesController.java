...
//			SecurityUtil.removeCacheBySystemId( res.getSystemId());
	...
	public List<Resources> getSystemTreeData(HttpServletRequest request, HttpServletResponse response) throws Exception {
		long systemId = RequestUtil.getLong(request, "systemId", 0);
		List<Resources> list = resourcesService.getBySystemId(systemId, Resources.FROMTYPE_SERVER);
	...
	public List<Resources> getSysRolResTreeChecked(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		List<Resources> resourcesList = resourcesService.getBySysRolResChecked(systemId, roleId, Resources.FROMTYPE_SERVER);
	...
	public void importXml(MultipartHttpServletRequest request, HttpServletResponse response) throws IOException
	{
		...
		ResultMessage resultMessage = null;
		try {
			Resources parentRes=resourcesService.getById(resId);
			resourcesService.importXml(fileLoad.getInputStream(),parentRes);
			List<String> msgList=MessageUtil.getMsg(true);
			if (msgList!=null&&msgList.size()>0){
				resultMessage = new ResultMessage(ResultMessage.Fail,msgList.get(0));
			}else {
			...
			List<String> strList = MessageUtil.getMsg();
			SysAuditThreadLocalHolder.putParamerter("isAdd", false);
			if (strList!=null&&strList.size()>0) {
				resultMessage = new ResultMessage(ResultMessage.Fail,"导入失败:" + StringUtil.join((String[]) strList.toArray(),","));
				response.getWriter().print(resultMessage);
			} else {
				String message = ExceptionUtil.getExceptionMessage(ex);
				resultMessage = new ResultMessage(ResultMessage.Fail, message);
	...
	public ModelAndView sortList(HttpServletRequest request,HttpServletResponse response) throws Exception{
		...
		List<Resources> list = resourcesService.getByParentId(resId, Resources.FROMTYPE_SERVER);
	...
	public ModelAndView tab(HttpServletRequest request,HttpServletResponse response,
		...
		mv.setViewName("/platform/system/resourcesTab");
