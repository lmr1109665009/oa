...
/**
 * 对象功能:控件Service类。 用于在freemaker模版中使用。 开发公司:广州宏天软件有限公司 开发人员:xwy 创建时间:2012-02-10 10:48:16
 */
@Service
public class BpmFormControlService {
	...
	public String getSubField(String html, Map<String, Map<String, Map>> model, Map<String, Map<String, String>> permission) {
		Document doc = Jsoup.parse(html);
		Element element = doc.select("[name]").get(0);
		
		String fieldName=element.attr("name").split(":")[2];
		String tableName=element.attr("name").split(":")[1].toLowerCase();
		SubTable subTable = (SubTable) model.get("sub").get(tableName);
		String val=(String) subTable.getRow().get(fieldName);
	...
	private static String getOptionValue(String html, String value, String extLink) {
		...
		if (sQuery != null && sQuery != "") {
			...
			/*
			 * }else{ if (BeanUtils.isNotEmpty(selectElement)) { return html; } Elements inputObj=doc.select("input"); String dicCom=inputObj.attr("class"); if ("dicComboTree".equals(dicCom)) { return html; } return extLink; }
			 */
		}
	...
	public String getLink(String fieldName, String html, Map<String, Map<String, Map>> model, Map<String, Map<String, String>> permission) {
		...
		fieldName = fieldName.toLowerCase();
		// 可写权限
		if (permission.get("field") == null || permission.get("field").get(fieldName) == null || "w".equals(permission.get("field").get(fieldName)) || "b".equals(permission.get("field").get(fieldName))) {
			return html;
		} else {
			return NO_PERMISSION;
		}
	...
	public String getFieldRight(String fieldName, Map<String, Map<String, String>> permission) {
		...
		// 可写权限
		if (fieldRight == null || "w".equals(fieldRight)) {
			return "w";
	 ...
	 * @return
	 */
	public String getFieldMenuRight(String fieldName, Map<String, Map<String, String>> permission) {
		fieldName = fieldName.toLowerCase();
