...
import com.suneee.platform.dao.bpm.HistoryTaskInstanceDao;
...
public class ScriptImpl implements IScript {
	...
	public String getCurrentName() {
		SysUser sysUser=(SysUser) ContextUtil.getCurrentUser();
		if(sysUser==null) return "";
		return ContextSupportUtil.getUsername(sysUser);
	}
	...
	public boolean isUserInOrg(String userId, String orgName) {
		...
		for (SysOrg sysOrg : list) {
			if (orgName.contains(sysOrg.getOrgName())) {
				return true;
			}
		}
		return false;
	...
	public boolean isUserInOrg(Long userId, String orgName){
		String strUserId = userId.toString();
		return isUserInOrg(strUserId,orgName);
	}
	...
	public String getFullNameByAccounts(String accouts){
		...
		for(SysUser s : sysUsers) {
			fullNames.append(ContextSupportUtil.getUsername(s));
			fullNames.append(",");
		}
	 ...
	 * 获取用户领导id集合。
	 * 脚本使用方法：
	 * scriptImpl.getLeaderByUserId(String userId);
	 * @param userId
	 * @return
	 * pengfeng
	 */
	public Set<String> getLeaderByUserId(String userId) {
		if(null==userId){
			return new HashSet<String>();
		}
		Long userIdL = Long.parseLong(userId);
		return getLeaderByUserId(userIdL);
	}
	/**
	 * 获取当前用户领导id集合。
	 * @param userId
	...
	public String getMgrByOrgIds(){
		...
		return ContextSupportUtil.getUsername(sysUser.get(0));
	}
	...
	public String getUserFullnameByCurOrgRoleAlias(String roleNameAlias){
		...
		return sysUser==null?"":ContextSupportUtil.getUsername(sysUser);
	}
	...
	public void validDataExist(BpmFormData formData,String fieldName,String messages){
		...
		Integer rtn= jdbcTemplate.queryForObject(sql,Integer.class, aryObj);
		if(rtn>0){
