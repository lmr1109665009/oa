...
//			taskOpinion.setExeFullname(sysUser.getFullname());
	...
	private void sendMessage(BpmTaskExe bpmTaskExe,String informType, Integer userType, Integer agentUseType,
		...
		taskMessageService.sendMessage(curUser, receiverUserList, informType, msgTempMap, bpmTaskExe.getSubject(), opinion, taskId, bpmTaskExe.getRunId(),null);
	...
	taskMessageService.sendMessage(curUser, receiverAgeUserList, informType, msgAgeTempMap, bpmTaskExe.getSubject(), opinion, taskId, bpmTaskExe.getRunId(),null);
		}
	...
	public ProcessRun doCancel(BpmTaskExe bpmTaskExe, SysUser sysUser,
		...
		// 状态修改为取消。
		if(bpmTaskExe.getAssignType()==1) {
			taskOpinion.setCheckStatus(TaskOpinion.STATUS_AGENT_CANCEL);
		}else{
...
//		taskOpinion.setExeFullname(curUser.getFullname());
	...
	public void cancel(BpmTaskExe bpmTaskExe, SysUser sysUser, String opinion, String informType) throws Exception {
		if(BpmTaskExe.TYPE_MUTIL_ASSIGNEE.equals(bpmTaskExe.getAssignType())){
	...
	public void cancelMutilLevel(BpmTaskExe bpmTaskExe, SysUser sysUser, String opinion, String informType) throws Exception {
		...
		QueryFilter filter = new QueryFilter(com.suneee.eas.common.utils.ContextUtil.getRequest());
		filter.setFilters(params);
		List<BpmTaskExe> taskList = getAll(filter);
		if(BeanUtils.isNotEmpty(taskList)){
			BpmTaskExe taskExe=taskList.get(0);
			// 继续对下级转办进行取消操作
			SysUser user = sysUserService.getById(taskExe.getAssigneeId()) ;
			cancelMutilLevel(taskExe, user, opinion, informType);
		}
		doCancel(bpmTaskExe, sysUser, opinion, informType);
	}
...
//				bpmTaskExe.setExeUserName(sysuer.getFullname());
				bpmTaskExe.setExeUserName(ContextSupportUtil.getUsername(sysuer));
	...
	public List<BpmTaskExe> getMobileAccordingMattersList(QueryFilter filter){
		return dao.getMobileAccordingMattersList(filter);
	}
	 ...
	 * @return
	 */
	public List<ProcessRunAmount> getAccordingMattersCount(QueryFilter filter) {
		return (List<ProcessRunAmount>) dao.getAccordingMattersCount(filter);
	}
	...
	public boolean isAssigneeTask(TaskEntity taskEnt,BpmDefinition bpmDefinition){
		...
		//退回到发起人 、 退回 、已经转办的任务不能在转办      《？？？？？？    以下还木有成功实现 退回不能转办的判定。？？？？？》
		//现改为已经转办的任务可进行多级转办
	...
	public List<BpmTaskExe> accordingMattersListOrgCode(QueryFilter filter) {
		return dao.accordingMattersListOrgCode(filter);
	}
