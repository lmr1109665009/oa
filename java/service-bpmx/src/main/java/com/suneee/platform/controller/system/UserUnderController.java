...
public class UserUnderController extends BaseController
{
	 ...
	 * @throws Exception
	 */
	@RequestMapping("getLeaderAndUnderUser")
	@Action(description="查询用户的上级和下属用户", detail="根据用户中心用户ID查询用户的上级和下属用户", execOrder=ActionExecOrder.AFTER, exectype="操作日志", ownermodel=SysAuditModelType.USER_MANAGEMENT)
	@ResponseBody
	public ResultVo getLeaderAndUnderUser(HttpServletRequest request, HttpServletResponse response) throws Exception{
		Long ucUserid = RequestUtil.getLong(request, "ucUserid");
		if(ucUserid == 0){
			logger.error("根据用户中心用户ID查询用户的上级和下属用户失败：请求参数不能为空！");
			return new ResultVo(ResultVo.COMMON_STATUS_FAILED, "查询用户的上级和下属用户失败：请求参数错误！");
		}
		
		try {
			List<Map<String, Object>> leaders = userUnderService.getLeaderByUcUserid(ucUserid);
			List<Map<String, Object>> userUnders = userUnderService.getUnderUserByUcUserid(ucUserid);
			Map<String, Object> result = new HashMap<String, Object>();
			result.put("leaders", leaders);
			result.put("userUnders", userUnders);
			return new ResultVo(ResultVo.COMMON_STATUS_SUCCESS, "查询用户的上级和下属用户成功！", result);
		} catch (Exception e) {
			logger.error("根据用户中心用户ID查询用户的上级和下属用户失败：" + e.getMessage(), e);
			return new ResultVo(ResultVo.COMMON_STATUS_FAILED, "查询用户的上级和下属用户失败：" + e.getMessage());
		}
	}
}
