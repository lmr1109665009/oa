...
import com.suneee.platform.dao.bpm.BpmNodeButtonDao;
import com.suneee.platform.model.bpm.BpmButton;
import com.suneee.platform.model.bpm.BpmDefinition;
import com.suneee.platform.model.bpm.BpmNodeButton;
import com.suneee.platform.model.bpm.BpmNodeButtonXml;
...
public class BpmNodeButtonService extends BaseService<BpmNodeButton> {
	...
	private void initNodeId(String actDefId, Long defId, String nodeId,
			boolean isSignTask,boolean isFirstNode,boolean isGateWay, List<BpmButton> list) throws Exception {
...
//					if((2==button.getType() || 4==button.getType()) && 1==button.getInit()){
					if(2==button.getType() && 1==button.getInit()){
						dao.add(new BpmNodeButton(actDefId, defId, nodeId, button.getText(), button.getOperatortype(), nodetype));
					}
				}
			}else if(isGateWay){
				for (BpmButton button:list){
					if(4==button.getType()&& 1==button.getInit()){
						dao.add(new BpmNodeButton(actDefId, defId, nodeId, button.getText(), button.getOperatortype(), nodetype));
					}
				}
			}
...
//					if((3==button.getType() || 4==button.getType()) && 1==button.getInit()){
					if(3==button.getType() && 1==button.getInit()){
						dao.add(new BpmNodeButton(actDefId, defId, nodeId, button.getText(), button.getOperatortype(), nodetype));
					}
				}
			}
	...
	public void init(Long defId, String nodeId) throws Exception {
		...
		if (isStartForm) {
			...
			//测试获取上一节点
			boolean isGateWay = NodeCache.isGateWay(actDefId, nodeId);
			initNodeId(actDefId, defId, nodeId, isSignTask, isFirstNode,isGateWay, list);
		}
	}
	/**
	 * 初始化流程的全部按钮。
	...
	public void initAll(Long defId) throws Exception {
		...
		for (Iterator<Map.Entry<String, FlowNode>> it = set.iterator(); it.hasNext();) {
			...
			boolean isGateWay = NodeCache.isGateWay(actDefId, entry.getKey());
			// 节点按钮。
			initNodeId(actDefId, defId, entry.getKey(), isSignTask,isFirstNode,isGateWay, list);
		}
	}
