...
public class TaskDao extends BaseDao<TaskEntity>
{
	...
	public List<Map<String, Object>> getAllTasks(QueryFilter queryFilter){
		List<Map<String, Object>> list = this.getListBySqlKey("getAllTasks", queryFilter);
		return list;
	}
	 ...
	 * @return
	 */
	public List<?> getProcessTaskCount(QueryFilter filter){
		return this.getBySqlKey("getAllTasksCount", filter);
	}
	 ...
	 * */
	@SuppressWarnings("unchecked")
	public List<ProcessTask> getAllMyTaskByDefIds(Map<String, Object> params, PageBean pageBean){
		 List list = this.getBySqlKey("getAllMyTaskByDefIds", params, pageBean);
		 return list;
	}

	public List<TaskEntity> getMyTaskss(Long currentUserId, QueryFilter queryFilter) {
		queryFilter.getFilters().put("userId",currentUserId);
		return getBySqlKey("getAllMyTasks", queryFilter);
	}
}
