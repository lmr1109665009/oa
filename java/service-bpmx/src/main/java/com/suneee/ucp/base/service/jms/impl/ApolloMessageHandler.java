...
//		content = content + "：" + url;
		...
		var.put("send",model.getSendUser());
		var.put("receiver",model.getReceiveUser());
		var.put("runtimeEnv",vars.get("runtimeEnv"));
		var.put("system",vars.get("system"));
		targets.add(receiver);
		targets.add(var);
		EventUtil.publishApolloMessageEvent(ApolloMessage.MSG_TYPE_APPROVAL, targets,
				ApolloMessage.TARGET_TYPE_PRIVATE, Integer.valueOf(vars.get("status").toString()), contentJson.toString());
