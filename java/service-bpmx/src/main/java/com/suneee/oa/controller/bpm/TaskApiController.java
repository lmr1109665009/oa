...
public class TaskApiController {
    ...
    public Object startFlow(@RequestBody Map<String, Object> map, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if(isMobile==null) {
            ...
            //根据userId对用户进行判断
            if (newUser == null) {
                newUser = processRunService.getUseridByAliasname(account.toString());
                if(newUser==null){
        ...
        try {
                ...
                if (destTaskIds != null) {
                    ...
                    for (int i = 0; i < destTaskIds.length; i++) {
                        ...
                        if(userIds!=null){
                            ...
                            //filter.addFilter("searchCondition",userIds);
                            SysUser nextUser=new SysUser();
                            List<SysUser> sysUsers = sysUserService.getByAccounts(userIds.toString());
                            if(sysUsers.size()>0)  nextUser =sysUsers.get(0);
            ...
            String comefrom = (String) comeFrom;
            BpmDefinition bpmDefinition = bpmDefinitionService.getByActDefId(processRun.getActDefId());
            GlobalType globalType = globalTypeService.getById(bpmDefinition.getTypeId());
            PerOffice po = new PerOffice();
            po.setComFrom(comefrom);
            po.setCreateBy(ContextSupportUtil.getCurrentUserId());
            po.setRunId(processRun.getRunId());
            po.setDefId(bpmDefinition.getDefId());
            po.setTypeName(globalType.getTypeName());
            po.setActDefId(bpmDefinition.getActDefId());
            po.setEnterpriseCode(globalType.getEnterpriseCode());
            bpmDefinitionService.save(po);
            return resultMap;
        } catch (Exception ex) {
