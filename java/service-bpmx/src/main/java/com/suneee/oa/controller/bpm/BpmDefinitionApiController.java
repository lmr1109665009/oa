...
public class BpmDefinitionApiController extends MobileBaseController {
	...
	public ResultVo myListWithoutEmpty(HttpServletRequest request,
		...
		for(Object key:map.keySet()){
			...
			if(entryList.size()!=0){
				String typeName = ((GlobalType)key).getTypeName();
				String bracket = typeName.substring(typeName.indexOf("("),typeName.indexOf(")")+1);
				typeName = typeName.replace(bracket, "("+entryList.size()+")");
				((GlobalType) key).setTypeName(typeName);
