...
<#if field.fieldType == "varchar"><#---字符串类型-->
	<#switch field.controlType>
		...
		<#break>
		<#case 3><#--数据字典-->
				<input lablename="${field.fieldDesc}" class="dicComboTree" nodeKey="${field.dictType}" value="${r"${"}${table.variable.classVar}.${field.fieldName}${r"}"}" validate="{empty:false<#if field.isRequired == 1>,required:true</#if><#if field.isWebSign == 1>,isWebSign:true</#if>}" name="${field.fieldName}" height="200" width="248"/>
		<#break>
		<#case 4><#--人员选择器(单选)-->
		...
		<#case 9><#--文件上传-->
				<div  name="div_attachment_container"  >
					...
					<a href="javascript:;" field="${field.fieldName}" class="link selectFile" atype="select"  onclick="<#if field.getPropertyMap().directUpLoad=="1">AttachMent.directUpLoadFile(this);<#else>AttachMent.addFile(this);</#if>" ></a>
				</div>	
		<#break>
		<#case 10><#--富文本框-->
