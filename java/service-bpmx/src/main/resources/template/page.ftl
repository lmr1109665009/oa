...
<div class="panel-page">		
		...
		<#if (showExplain) >
			<div class="l-bar-group" >
				<#if (pageBean.totalCount==0)>
                    <span class="l-bar-text">显示记录从0到0，总数 0条</span>
				<#else>
	...
	<div class="l-bar-group-text">
		...
		<div class="l-bar-group">
			<span class="pcontrol">
			<#if (pageBean.totalCount==0)>
				<input size="4" value="0" style="width: 20px;text-align:center" maxlength="3" class="inputText pageInput" type="text" tableidcode="${tableIdCode}" id="navNum${tableIdCode}" name="navNum"/>
			<#else>
                <input size="4" value="${pageBean.currentPage}" style="width: 20px;text-align:center" maxlength="3" class="inputText pageInput" type="text" tableidcode="${tableIdCode}" id="navNum${tableIdCode}" name="navNum"/>
			</#if>
				<div class="l-bar-separator"></div>
				<span class="totalPage"> / <span>${pageBean.totalPage}</span></span>
			</span>
		</div>
		<div class="l-bar-separator"></div>
		<div class="l-bar-group">
