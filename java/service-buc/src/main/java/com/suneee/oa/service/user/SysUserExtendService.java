...
@Service
 public class SysUserExtendService extends UcpBaseService<SysUser> {
	...
	private static final String MESSAGE_STATUS_ADD= Constants.MESSAGE_STATUS_ADD;

	private static final String MESSAGE_STATUS_UPDATE= Constants.MESSAGE_STATUS_UPDATE;
...
//	public void save(SysUser sysUser, Long[] roleIds, String positions) throws Exception {
	...
	public void save(SysUser sysUser,String positions) throws Exception {
		...
		if (event == UserEvent.ACTION_ADD) {
			...
			stauts = MESSAGE_STATUS_ADD;
		}else {
			this.sysUserExtendDao.updateUser(sysUser);
			stauts=MESSAGE_STATUS_UPDATE;
		}
		...
		sendMsgCenterUtils.sendToUserInfoCenter(sysUser, stauts,AppConfigUtil.get(Constants.MESSAGE_USER_TOPIC));
	}

	public List<SysUser> getUserListByIds(Long[] ids){
		return sysUserExtendDao.getUserListByIds(ids);
	}
}
