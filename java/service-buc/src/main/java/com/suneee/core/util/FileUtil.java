...
public class FileUtil {
	 ...
	 * @param append
	 */
	public static void writeFile(String fileName, String content,String charset, boolean append){
		Writer out = null;
		try {
			createFolder(fileName, true);
			out = new BufferedWriter(new OutputStreamWriter(
	...
	public static void writeFile(String fileName, InputStream is)
			throws IOException {
		//String downloadUrl = com.suneee.eas.common.utils.FileUtil.getDownloadUrl(fileName, false);
...
//		String path=StringUtil.trimSufffix(AppUtil.getRealPath("/"), File.separator)   +File.separator +"WEB-INF"+File.separator+"classes"+File.separator;
		String path=AppUtil.getClasspath();
		return path;
	}
	 ...
	 * @return 文件的完整目录
	 */
	public static String createDateFilePath(String root,String dir, String fileName) {
		File one = null;
		Calendar cal = Calendar.getInstance();
		Integer year = cal.get(Calendar.YEAR); // 当前年份
		Integer month = cal.get(Calendar.MONTH) + 1; // 当前月份
		String fileDir=dir +  "/" + year +  "/" + month;
		one = new File(root+fileDir);
		if (!one.exists()) {
			one.mkdirs();
		}
		String extName = getFileExt(fileName);//文件后缀名
		return fileDir +"/" + System.currentTimeMillis() + "." + extName;
	}
