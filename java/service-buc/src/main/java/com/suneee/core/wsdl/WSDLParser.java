...
//		String wsdlURI = "http://www.webxml.com.cn/WebServices/WeatherWebService.asmx?wsdl";
		String wsdlURI = "http://hotent.yicp.io:9080/bpm/service/UserDetailsService?wsdl";
		try {
	...
	private String elementFormDefault = XmlSchemaForm.QUALIFIED.toString();
	...
	private void processImport(Definition defintion) throws WSDLException {
		...
		if(BeanUtils.isNotEmpty(extensibilityElements)){
			Object object = extensibilityElements.get(0);
			SchemaImpl schema = (SchemaImpl)object;
			if(BeanUtils.isNotEmpty(schema)){
				Element element = schema.getElement();
				if(BeanUtils.isNotEmpty(element)){
					//获取构建soap的入参是否需要namespace前缀
					elementFormDefault = element.getAttribute("elementFormDefault"); 
				}
			}
		}
		while (keys.hasNext()) {
	...
	private void processService(ServiceImpl service, String wsdlUrl) {
		...
		serviceInfo.setNeedPrefix(XmlSchemaForm.QUALIFIED.toString().equals(elementFormDefault));
