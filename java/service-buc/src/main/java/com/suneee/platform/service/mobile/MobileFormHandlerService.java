...
import com.suneee.platform.model.mobile.MobileFormData;
import com.suneee.platform.model.mobile.MobileSubTableData;
...
public class MobileFormHandlerService extends
	...
	public MobileFormData getMobileForm(MobileFormData mobileFormData,
		...
		if (bpmNodeSet.getFormType().shortValue() == 1) {
			isExtForm = true;
			formUrl = bpmFormDefService.getFormUrl(taskId, actDefId, nodeId,
		...
		if (!isExtForm) {
			...
			mobileFormData = this.getMobileFormData(bpmFormDef, pkValue,
					processRun, mobileFormData,nodeId,ctxPath);
		}
	...
	public MobileFormData getMobileFormData(BpmFormDef bpmFormDef,
			String pkValue, ProcessRun processRun, MobileFormData mobileFormData, String nodeId,String ctxPath)
			throws Exception {
		if (BeanUtils.isEmpty(bpmFormDef))
			return mobileFormData;

		Long tableId = bpmFormDef.getTableId();
		if (BeanUtils.isEmpty(tableId) || tableId.longValue() == 0L) return mobileFormData;
		
		BpmFormTable table=bpmFormDef.getBpmFormTable();
		...
		Map<String, Object> permission= bpmFormRightsService.getByFormKeyAndUserId(bpmFormDef, processRun.getActDefId(), nodeId, parentActDefId,false);
	...
	private MobileFormData getFormData(BpmFormTable table, String pkValue,
			String actInstId, Map<String, String> tableMap, Map<String, Object> permission,String ctxPath) throws Exception {
		MobileFormData formData = new MobileFormData();
		Long tableId = table.getTableId();
		BpmFormTable formTable=bpmFormTableService.getByTableId(tableId, 1);
		// 取出表单数据
		BpmFormData bpmFormData = bpmFormHandlerService.getBpmFormData(formTable,
				pkValue, actInstId, null, null,false,false,false,(short)0);
		...
		// 设置子表的数据
		this.setSubDataList(formData, bpmFormData, tableMap, formTable);
	...
	private Map getOptionPermission(
			Map<String, Object> permission) {
		return (Map) permission.get("opinion");
	}
	...
	private void setSubDataList(MobileFormData formData,
			BpmFormData bpmFormData, Map<String, String> tableMap, BpmFormTable formTable) {
		// 手机表单子表
		...
		// 根据主表Id获取子表列表
		List<BpmFormTable> subTableList =formTable.getSubTableList();
		if(BeanUtils.isEmpty(subTableDataList)) return;
		for (int i = 0; i < subTableList.size(); i++) {
			...
			List<BpmFormField> subFieldList = subFormTable.getFieldList();
	...
	public MobileFormData getMobileFormData(BpmFormDef bpmFormDef,
		...
		if (BeanUtils.isEmpty(tableId) || tableId == 0L)
			return mobileFormData;
		BpmFormTable table = bpmFormTableService.getByTableId(tableId, 1);
		Map<String, String> tableMap = this.parseTableMap(bpmFormDef, table);
		return this.getFormData(table, pkValue, tableMap,ctxPath);
	}
	...
	private MobileFormData getFormData(BpmFormTable table, String pkValue,
		...
		// 取出表单数据
		BpmFormData bpmFormData = bpmFormHandlerService.getBpmFormData(table,
				pkValue, null, null, null,false,false,false,(short)0);
		...
		formData.setOptions(bpmFormData.getOptions());
		// 设置子表的数据
		this.setSubDataList(formData, bpmFormData, tableMap, table);
		return formData;
	}
