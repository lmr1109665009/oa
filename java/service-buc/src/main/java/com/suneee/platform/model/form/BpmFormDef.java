...
public class BpmFormDef extends BaseModel implements Cloneable
{
	 ...
	 * 表单对应的表对象。
	 */
	protected BpmFormTable bpmFormTable;
	...
	public void setBpmFormTable(BpmFormTable table){
		this.bpmFormTable=table;
	}
	
	public BpmFormTable getBpmFormTable() {
		if(bpmFormTable==null){
			if(this.tableId==null || this.tableId.equals(0L)) return null;
			BpmFormTableService tableService= AppUtil.getBean(BpmFormTableService.class);
			bpmFormTable =tableService.getByTableId(this.tableId, 1);
			this.tableName=bpmFormTable.getTableName();
		}
		if(bpmFormTable != null)
			this.tableName=bpmFormTable.getTableName();
		return bpmFormTable;
