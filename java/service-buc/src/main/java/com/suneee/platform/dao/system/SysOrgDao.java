...
public class SysOrgDao extends BaseDao<SysOrg> {
    ...
    private String usernameMode;

    @Autowired
    public void setEnv(Environment env) {
        this.environment = env;
        this.usernameMode = env.getProperty("system.username-mode", "");
    }
    ...
    public void delByPath(String path) {
        ...
        params.put("updateId", ContextUtil.getCurrentUserId());
        params.put("path", path);
        this.update("delByPath", params);
    }
    ...
    public SysOrg getPrimaryOrgByUserId(Long userId) {
        ...
        return this.getUnique("getPrimaryOrgByUserId", params);
    }
     ...
     * @return
     */
    public List<SysOrg> getOrgByorgSupId(String orgSupId) {
        Map<String, Object> params = new HashMap<String, Object>();
        params.put("orgSupId", orgSupId);
        return this.getBySqlKey("getOrgByorgSupId", params);
    }
    ...
    public List<SysOrg> getUserListByOrgId(Long orgId) {
        List<SysOrg> userList = this.getBySqlKey("getUserListByOrgId", orgId);
        String[] userNames = null;
        for (SysOrg sysOrg : userList) {
            userNames = sysOrg.getOrgName().split(",");
            if (userNames.length > 0) {
                if (ContextSupportUtil.MODE_ALIAS.equals(usernameMode) && userNames.length == 2) {
                    sysOrg.setOrgName(userNames[1]);
                } else {
                    sysOrg.setOrgName(userNames[0]);
                }
            }
        }
        return userList;
    ...
    public SysOrg getByUserId(Long userId) {
        List<SysOrg> list = this.getBySqlKey("getByUserIdSecond", userId);
        if (list.size() == 0) {
            return new SysOrg();
        }
        return list.get(0);
    ...
    public List<SysOrg> getAllComp(QueryFilter queryFilter) {
        return this.getBySqlKey("getAllComp", queryFilter);
    }

    public Position getToUIdByPosition(Long userId) {
        Map map = new HashMap();
        map.put("userId", userId);
        return (Position) this.getOne("getToUIdByPosition", map);
    }
}