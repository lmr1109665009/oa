...
public class SysUserDao extends BaseDao<SysUser> {
     ...
     * 对象功能：根据查询条件查询用户(用于用户管理)
     */
    public List<SysUser> getUsersByQueryForCodeManage(QueryFilter queryFilter) {
        if (this.getDbType().equals(SqlTypeConst.ORACLE)
                || this.getDbType().equals(SqlTypeConst.SQLSERVER)
                || this.getDbType().equals(SqlTypeConst.MYSQL)) {
            return this.getBySqlKey("getUsersByQueryFilterForCodeManage",
                    queryFilter);
        } else {
            return this.getBySqlKey("getUsersByQueryFilterForCodeManage", queryFilter);
        }
    }
    ...
    public List<SysUser> getUsersByQueryFilter(QueryFilter queryFilter) {
        if (this.getDbType().equals(SqlTypeConst.ORACLE)
                || this.getDbType().equals(SqlTypeConst.SQLSERVER)
                || this.getDbType().equals(SqlTypeConst.MYSQL)) {
            return this.getBySqlKey("getUsersByQueryFilter",
                    queryFilter);
        } else {
            return this.getBySqlKey("getUsersByQueryFilter", queryFilter);
        }
    }
    ...
    public boolean isAccountExist(String account, Long userId) {
        ...
        Integer rtn = (Integer) this.getOne("isAccountExist", params);
        return rtn > 0;
    }
    ...
    public boolean isEmailExist(String email, Long userId) {
        ...
        params.put("userId", userId);
        Integer rtn = (Integer) this.getOne("isEmailExist", params);
        return rtn > 0;
    }
    ...
    public boolean isMobileExist(String mobile, Long userId) {
        ...
        params.put("userId", userId);
        Integer rtn = (Integer) this.getOne("isMobileExist", params);
        return rtn > 0;
    }
    ...
    public SysUser getByUserId(Long supId) {
        List<SysUser> list = this.getBySqlKey("getById", supId);
        if (list.size() == 0) {
            return new SysUser();
        }
        return list.get(0);
    ...
    public SysUser getSupUserByUserId(Long orgId) {
        List<SysUser> list = this.getBySqlKey("getSupUserByUserId", orgId);
        if (list.size() == 0) {
            return new SysUser();
        }
        return list.get(0);
    ...
    public List<SysUser> getByFullName(String fullname) {
        ...
        return this.getBySqlKey("getByFullName", fullname);
    }

    public SysUser getByMobile(String mobile) {
        return (SysUser) this.getOne("getByMobile", mobile);
    }
    ...
    public void addheadImg(Long userId, String path) {
        ...
        param.put("headImgPath", path);
        this.update("addheadImg", param);
    }

    public SysUser getByUcUserId(Long ucUserId) {
        List<SysUser> list = this.getBySqlKey("getByUcUserid", ucUserId);
        if (list.size() == 0) {
            return new SysUser();
        }
        return list.get(0);
    ...
    public Boolean updateBySysUser (SysUser sysUser){
        sysUser.setUpdatetime(new Date());
        int updateBySysUser = this.update("updateBySysUser", sysUser);

        return updateBySysUser>0&&updateBySysUser<2;
    }

    public Boolean setUserPosOrg(Map param) {
        int setUserPosOrg = this.insert("setUserPosOrg",param);
        return setUserPosOrg>0&&setUserPosOrg<2;
    }
}