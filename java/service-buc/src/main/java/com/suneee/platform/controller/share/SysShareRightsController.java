...
package com.suneee.platform.controller.share;
...
import com.suneee.platform.service.share.SysShareRightsService;
...
public class SysShareRightsController extends BaseController
{
	...
	public Object save(HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		...
		SysShareRights sysShareRights = null;
		try{
			sysShareRights = GsonUtil.toBean(jsonStr, SysShareRights.class);
			if(BeanUtils.isNotIncZeroEmpty(sysShareRights.getId())){
			...
			JSONObject  joSource = JSONObject.fromObject(sysShareRights.getSource());
			String sourceNames = joSource.getString("names"); //共享原名称
			JSONObject  joTarget = JSONObject.fromObject(sysShareRights.getTarget());
			String targetNames =joTarget.getString("names");
			JSONObject  joItem = JSONObject.fromObject(sysShareRights.getShareItem());
			String shareItems = joItem.getString("names");
			String shareType = joItem.getString("type");
			String txt = "["+sourceNames + "] 将 ["+shareType+"] 类型的 ["+shareItems+"] 权限共享给 ["+targetNames+" ]";
			SysAuditThreadLocalHolder.putParamerter("txt", txt);
