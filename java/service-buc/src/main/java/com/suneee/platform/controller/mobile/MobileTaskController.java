...
import com.suneee.platform.model.mobile.MoblieImage;
...
import com.suneee.platform.service.mobile.MobileTaskService;
import com.suneee.platform.service.system.GlobalTypeService;
import com.suneee.platform.service.system.IndexService;
...
public class MobileTaskController extends MobileBaseController {
	...
	@Action(description = "所有流程接口")
	@ResponseBody
	public Object allProcessList(HttpServletRequest request) throws Exception {
		...
		if (StringUtils.isNotEmpty(processName))
			filter.getFilters().put("processName", "%"+processName + "%");
		List<ProcessRun> list = processRunService.getAllFlowsList(filter);
		return this.getPageList(list, filter);
	}
...
//			processRunService.saveRestartTask(taskId,runId,new Long(processRun.getActInstId()),informTypes,voteAgree,voteContent);
	...
	@RequestMapping("processTaskInfo")
	@Action(description = "我的各个流程状态统计")
	@ResponseBody
	public Object processTaskInfo(HttpServletRequest request, HttpServletResponse response) throws Exception {
		return indexService.mobileProcessStatusInfo();
	}
	...
	@Action(description = "流程搜索接口")
	@ResponseBody
	public Object searchFlows(HttpServletRequest request) throws Exception {
		QueryFilter filter = new QueryFilter(request, true);
