...
import com.suneee.core.util.SqlDateFormat;
...
public class SqlParseUtil {
	...
	public static String getSortSql(String sortField,String orderSeq){
		...
		if(sortField.endsWith(",")){
			return " order by " +StringUtil.trimSufffix(sortField, ",");
		}
	...
	public static List<SQLClause> getQueryParameters(SysQueryView queryView, boolean isForm){
		...
		for(SQLClause clause:conditionFields){
			int valFrom=isForm?SQLClause.VALUEFROM_INPUT:SQLClause.VALUEFROM_FORM;
	...
	private static void getNormalFilterResult(FilterJsonStruct filter,
		...
		switch (optType) {
			...
			case 1:
				...
				if (StringUtils.isNotEmpty(judgeVal2)) {
					...
					if (StringUtils.isNotEmpty(script))
						script = script + SysQueryView.CONDITION_AND;
			...
			case 3:
				// 条件一
				if (StringUtils.isNotEmpty(judgeVal1)) {
					String val1 = SqlDateFormat.convertDateFormat(filter.getDatefmt(), judgeVal1, dbType) ;
					script = getCompareScript(filter.getJudgeCon1(),flowvarKey, val1, filter.getOptType());
				}
				// 条件二
				if (StringUtils.isNotEmpty(judgeVal2)) {
					String val2 = SqlDateFormat.convertDateFormat(filter.getDatefmt(), judgeVal1, dbType) ;
			...
			case 4:
				...
				for (String val : vals) {
					if (StringUtils.isNotEmpty(script)){
						script += SysQueryView.CONDITION_AND;
					}
	...
	private static void getCluaseSQL(SysQueryView queryView,
		...
		if(StringUtil.isEmpty(joinType)) joinType="and";
