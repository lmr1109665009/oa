...
public class BpmService implements IScript
{
	 ...
	 */
	public void transTo(TaskEntity task, String toNode) throws IOException {
		// 获取流程定义对象
	 ...
	 * @return
	 */
	private boolean handForkTask(TaskEntity task,String destNodeId,ProcessDefinitionEntity processDefinition) throws IOException {
...
//		((ProcessEngineConfigurationImpl)processEngineConfiguration).getDeploymentCache().clearProcessDefinitionEntity();
	 ...
	 * @return
	 */
	public List<Map<String, Object>> getAllTasks(QueryFilter queryFilter){
		return taskDao.getAllTasks(queryFilter);
	}
	...
	public List<NodeTranUser> getNodeUserMap(String processDefinitionId,String instanceId, List<FlowNode> nextFlowNodes,Long curUserId,Long preUserId ,boolean canChoicePath,Map<String,Object> vars ){
		...
		if(null!=instanceId) {
			...
			if (null != processRun.getRunId()) {
				try {
					//获取审批历史中的数据
					List<TaskOpinion> list = taskOpinionService.getByRunId(processRun.getRunId());
					//处理审批历史中相同的审批数据
					List<TaskOpinion> opinions = setNewOpinionList(list);
					//把已审批节点的人员信息放入要选择的节点中
					nodeList = getTaskerByStack(nodeList, opinions);
				} catch (Exception e) {
					e.printStackTrace();
					return null;
				}
...
//			Long duration = calendarAssignService.getRealWorkTime(startTime, endTime,userId );
		...
		if (BeanUtils.isNotEmpty(taskOpinionList)) {
			...
			if(processRun.getStatus().equals(ProcessRun.STATUS_RUNNING)){//流程运行时
				...
				List<TaskExecutor> taskExecutorList = bpmNodeUserService.getExeUserIds(processRun.getActDefId(),
						actInstId, nodeId, processRun.getCreatorId().toString(), String.valueOf(vars.get("startUser")), vars);
	 ...
	 * @return
	 */
	public List<ProcessRunAmount> getProcessTaskCount(QueryFilter filter) {
		return (List<ProcessRunAmount>) taskDao.getProcessTaskCount(filter);
	}
	 ...
	 * @throws Exception
	 */
	public List<TaskOpinion> setNewOpinionList(List<TaskOpinion> list) throws Exception{
		Iterator<TaskOpinion> it = list.iterator();
		while (it.hasNext()){
			TaskOpinion task = it.next();
			//把撤销状态的历史信息从列表中删除
			if(task.getExeUserId()==null||task.getTaskKey()==null||task.getCheckStatus()==4){
				it.remove();
			}
		}
		list=new CopyOnWriteArrayList<TaskOpinion>(list);
		for(TaskOpinion opinionItem1:list){
			for(TaskOpinion opinionItem2:list){
				if(opinionItem1.getTaskKey().equals(opinionItem2.getTaskKey())&&opinionItem1.getOpinionId()!=opinionItem2.getOpinionId()) {
					if (opinionItem1.getExeUserId().equals(opinionItem2.getExeUserId())) {
						list.remove(opinionItem2);
						continue;
					} 
					if (opinionItem1.getTaskKey().contains("UserTask")&opinionItem1.getEndTime()!=null&opinionItem2.getEndTime()!=null) {
						//如果相同节点有两个不同审批人的记录，删除最先的那条数据
						if (opinionItem1.getEndTime().after(opinionItem2.getEndTime())) {
							list.remove(opinionItem2);
							continue;
						}
						list.remove(opinionItem1);
					}
				}
			}
		}
	...
	/**
	 * 将审批历史中的已审批人替换掉流程节点的审批人
	 * @param nodeTranUserList
	 * @param list
	 * @return
	 * @throws Exception
	 */
	public List<NodeTranUser> getTaskerByStack(List<NodeTranUser> nodeTranUserList,List<TaskOpinion> list) throws Exception{
		for (NodeTranUser nodeTranUser:nodeTranUserList) {
			Set<NodeUserMap> userSet = nodeTranUser.getNodeUserMapSet();
			if(nodeTranUser.getNodeUserMapSet().size()>0){
				Iterator<NodeUserMap> iterator =userSet.iterator();
				while (iterator.hasNext()){
					NodeUserMap mapSet = iterator.next();
					String nodeId = nodeTranUser.getNodeId();
					Set<TaskExecutor>  executorSet= new LinkedHashSet<TaskExecutor>();
					for (TaskOpinion opinion:list) {
						if(nodeId.equals(opinion.getTaskKey())){
							executorSet =  changeTaskExecutors(executorSet,opinion);
							mapSet.setTaskExecutors(executorSet);
						}
					}
				}
			}
		}
		return nodeTranUserList;
	}

	public Set<TaskExecutor> changeTaskExecutors(Set<TaskExecutor>  executorSet,TaskOpinion opinion) throws Exception{
	...
	public String getActDefIdByDeployId(String deployId){
		return bpmDao.getActDefIdByDeployId(deployId);
	}
}