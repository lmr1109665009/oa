...
public class GlobalTypeController extends BaseController {
	...
	public void del(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		try {
			...
			if (typeList.size()>1){
				resultMessage.setMessage("无法删除，请先转移或删除子分类！");
			...
			switch (globalType.getCatKey()){
				case GlobalType.CAT_FLOW:
					...
					if (bpmDefinitionList.size()>0){
						...
						if (bpmDefinitionList.size()>0){
							...
							resultMessage.setMessage("无法删除，其他管理员在该流程类名下挂有流程！");
						}
						writeResultMessage(response.getWriter(), resultMessage);
						return;
					}
				...
				case GlobalType.CAT_FORM_TABLE:
					...
					break;
				default:
					resultMessage.setMessage("参数不对，非法操作！");
					writeResultMessage(response.getWriter(), resultMessage);
					return;
			}
			...
			resultMessage.setMessage("删除分类成功");
			writeResultMessage(response.getWriter(), resultMessage);
		} catch (Exception e) {
