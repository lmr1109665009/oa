...
public class EditParseHandler extends AbstractParseHandler {
	...
	private void parseDesign(Document doc, Map<String, Object> param) {
		...
		for (Element external : mainExternals) {
			//下面那段是过滤掉子表字段--------------->
			Elements es = external.parents();
			boolean b = false;
			for (Element e : es) {
				if (e.hasClass("subtable")) {
					b = true;
				}
			}
			if (b) {
		...
		for (Element subtableElement : subtableElements) {
			...
			String popupTyle = "";
			Element listRowElement = null;
			if (subtableElement.select("tr.listRow").size() > 0) {
				// 拼装html，下面的代码开始处理子表弹出框或者编辑模式的html拼装
				listRowElement = subtableElement.select("tr.listRow").get(0);
			} else {
				//这是模块模式
				listRowElement = subtableElement.select("[formtype]").get(0);
				if (listRowElement.select("table.blocktable").size() > 0) {
					popupTyle = "kuai";
				}
			}
			...
			String idHtml = "<input name=\"s:" + subTable.getTableName() + ":" + subTable.getPkField().toLowerCase() + "\" type=\"hidden\" pk=\"true\" value=\"\" />";
			// 添加id
			if (popupTyle.equals("kuai")) {
				listRowElement.append(idHtml);
			} else {
				subtableElement.append(idHtml);
			}
			...
			if (popupTyle.equals("kuai")) {
				subtableElement.append(newListRowElement.toString());
			} else {
