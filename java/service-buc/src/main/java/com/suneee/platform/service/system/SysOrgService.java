...
public class SysOrgService extends BaseService<SysOrg> {
	...
	private IOrgService wxOrgService;
	...
	public void delById(Long id) throws IOException {
		...
		for(SysOrg org : sysOrgs){
			org.setIsDelete((short)1);
			org.setUpdatetime(new Date());
			org.setUpdateId(ContextUtil.getCurrentUserId());
			SendMsgCenterUtils sendMsgCenterUtils = new SendMsgCenterUtils();
			//同步到消息中心
			ResponseEntity<TopicsRecord> topicsRecordResponseEntity = sendMsgCenterUtils.sendToUserInfoCenter(org, Constants.MESSAGE_STATUS_DELETE, AppConfigUtil.get(Constants.MESSAGE_ORG_TOPIC));
		}
		EventUtil.publishOrgEvent(OrgEvent.ACTION_UPD, sysOrgs);
	}
	/**
	...
	public void addOrg(SysOrg sysOrg) throws Exception{
		...
		List<SysOrg> list = new ArrayList<SysOrg>();
		list.add(sysOrg);
		EventUtil.publishOrgEvent(OrgEvent.ACTION_ADD, list);
		for (SysOrg org : list) {
			SendMsgCenterUtils sendMsgCenterUtils = new SendMsgCenterUtils();
	...
	public void updOrg(SysOrg sysOrg) throws Exception{
		...
		List<SysOrg> list = new ArrayList<SysOrg>();
		list.add(sysOrg);
		EventUtil.publishOrgEvent(OrgEvent.ACTION_UPD, list);
		for (SysOrg org : list) {
			SendMsgCenterUtils sendMsgCenterUtils = new SendMsgCenterUtils();
	...
	public List<SysOrg> getUserListByOrgId(Long orgId){
		return dao.getUserListByOrgId(orgId);
	}

	public SysOrg getByUserId(Long userId) {
		SysOrg sysOrg = dao.getByUserId(userId);
		String orgCode = sysOrg.getOrgCode();
		if(orgCode==null||orgCode==""){
			if(sysOrg.getOrgSupId()!=null){
				SysOrg supOrg = dao.getById(sysOrg.getOrgSupId());
				String supCode = supOrg.getOrgCode();
				if(supCode!=null){
					sysOrg.setOrgCode(supCode);
				}else{
					sysOrg.setOrgCode(supOrg.getCode());
				}
			}else{
				sysOrg.setOrgCode(sysOrg.getCode());
			}
		}
			return sysOrg;				
	}
   //获取所有公司及以上
	public List<SysOrg> getAllComp(QueryFilter queryFilter){ return dao.getAllComp(queryFilter);}
	 ...
	 * @return
	 */
	public List<SysOrg> getByOrgIds(String orgIds){
		String[] stringIds = orgIds.split(",");
		List<Long> orgIdList = new ArrayList<>();
		for(String id:stringIds){
			orgIdList.add(Long.valueOf(id));
		}
		return dao.getByOrgIds(orgIdList);
	}
}
