 ...
 */
@Service
public class PieceModeTableParser extends TableAbstractParser {
	@Override
	public Object parse(ParserParam param, Element element) {
		...
		idE.attr("name", idStr);
		idE.val("");
		rowE.append(idE.toString());//添加都尾部
		...
		for(String id:subTable.getDataMap().keySet()){
			...
			String subtableId=subTable.getPkName().toLowerCase();
			if (map.get(subtableId)==null){
				subtableId=subtableId.toUpperCase();
			}
			String dataId = map.get(subtableId).toString();
			ide.val(dataId);
