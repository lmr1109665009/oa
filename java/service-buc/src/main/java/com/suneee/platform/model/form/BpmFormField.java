...
public class BpmFormField extends BaseModel implements Cloneable {
	...
	/**
	 * 分类，目前分类为：表单字段:0，流程变量字段:1，意见字段:2
	 */
	@XmlAttribute
	...
	/**
	 * 单选框,复选框,下拉框的默认选项，在生成表单模板解释上面用到
	 */
	public Map<String, Object> getIsDefault() {
		Map<String, Object> map = new LinkedHashMap<String, Object>();
		if (StringUtil.isEmpty(this.options)){
			return map;
		}
		if (StringUtil.isJson(this.options)) {
			...
			for (Object obj : jArray) {
				JSONObject jObject = (JSONObject) obj;
				if(!(jObject.containsKey("key")) || !(jObject.containsKey("isDefault"))){
					continue;
				}
				String key = jObject.getString("key");
				String radioDefault = jObject.getString("isDefault");
	...
	public Map<String, String> getPropertyMap() {
		...
		try {
			...
			while (it.hasNext()) {
				...
				if("scope".equals(key))value = value.replaceAll("\"", "'");
				map.put(key, value);
			}
		} catch (Exception ex) {
			ex.printStackTrace();
	 ...
	 * @throws Exception
	 */
	public String generateUniqueCode() throws Exception {
		String content="BpmFormField{" +
				"fieldName='" + fieldName + '\'' +
				", fieldType='" + fieldType + '\'' +
				...
				", dictType='" + dictType + '\'' +
				'}';
		return EncryptUtil.encryptMd5(content);
	}
