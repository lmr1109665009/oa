package com.suneee.ucp.mh.model.schedulecalendar;
 ...
 */
public class ScheduleCalendar extends UcpBaseModel{
	/**
	 * 
	 */
	private static final long serialVersionUID = 908518707790289046L;
	// 日程ID
	protected Long id;
	// 标题
	...
	public String getStart(){
		if(startTime != null){
			if(allDay == 1){
				return DateFormatUtil.format(startTime, DateUtils.FORMAT_DATE);
			}else{
				return DateFormatUtil.format(startTime, DateUtils.FORMAT_DATETIME);
			}
		}
		else{
			return "";
	...
	public String getEnd(){
		if(endTime != null){
			if(allDay == 1){
				return DateFormatUtil.format(endTime, DateUtils.FORMAT_DATE);
			}else{
				return DateFormatUtil.format(endTime, DateUtils.FORMAT_DATETIME);
			}
		}
		else{
			return "";
