...
@Controller
@RequestMapping("/mh/codeTable/areaManeger/")
public class AreaManegerController extends UcpBaseController {
	...
	public ModelAndView list(HttpServletRequest request,HttpServletResponse response) throws Exception{
		...
		for (CodeTable list : lists) {
			...
			String typename = globalService.getByNodeKey(nodekey).getTypeName();
	...
	public void del(HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		...
		try{
			...
			message = new ResultMessage(ResultMessage.Success, "删除区域行政副总成功!");
		}catch(Exception ex){
	...
	public void save(HttpServletRequest request,HttpServletResponse response,CodeTable codeTable) throws Exception{
		...
		String userName = codeTable.getUserName();
		if(userName.contains("-")){
			String items[] = userName.split("-");
			codeTable.setItemId(items[1]);
		}		
		try {
			codeTableService.save(codeTable);
				if(0==settingId){
					resultMsg="添加区域行政副总成功";
				}else{
					resultMsg="更新区域行政副总成功";
				}
