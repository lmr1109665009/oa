...
public class SumTag implements ITag {
	...
	public int[] parseTag(Object context, HSSFSheet sheet, HSSFRow curRow,
		...
		cellstr = cellstr.substring(cellstr.indexOf(getTagName()),
				cellstr.length());
				...
				if (null != model) {
					...
					if (null != value) {
						...
						if (!"".equals(whereName) && !"".equals(whereStr)) {
							Object where = ExcelParser.getValue(model,
									whereName);
							Object whereValue = null;
							if (whereStr.startsWith(ExcelParser.VALUED_DELIM)) {
								whereValue = ExcelParser.parseStr(context,
										whereStr);
							}
							if (null == whereValue)
								whereValue = whereStr;
							...
							if (null != where) {
								if (bEquals) {
									...
									if (where.toString().indexOf(
											whereValue.toString()) < 0) {
										continue;
									}
								}
							}
						...
						// sum the value
						if ("java.lang.Integer".equals(value.getClass()
								.getName())
								|| "java.lang.Double".equals(value.getClass()
										.getName())) {
