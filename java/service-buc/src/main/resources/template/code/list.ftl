		...
		<div class="panel-top">
			...
			<div class="panel-toolbar">
				<div class="toolBar">
					...
					<#if flowKey?exists>
					    <div class="l-bar-separator"></div>
						<div class="group"><a class="link run"  onclick="$.openFullWindow('<#noparse>${ctx}</#noparse>/platform/bpm/task/startFlowForm.ht?defId=${defId}');"><span></span>启动流程</a></div>
					</#if>
				</div>	
			</div>
			<div class="panel-search">
				<form id="searchForm" method="post" action="list.ht">
					<div class="row">
						...
						<span class="label">至: </span><input  name="Q_end${field.fieldName}_DL" class="inputText date" />
		    ...
		    <display:table name="${classVar}List" id="${classVar}Item" requestURI="list.ht" sort="external" cellpadding="1" cellspacing="1" class="table-grid">
				...
				<display:column title="管理" media="html" style="width:220px">
					...
					<c:if test="<#noparse>${</#noparse>${classVar}Item.runId==0}">
						...
						<a href="javascript:;" onclick="$.openFullWindow('<#noparse>${ctx}</#noparse>/platform/bpm/task/startFlowForm.ht?defId=${defId}&businessKey=<#noparse>${</#noparse>${classVar}Item.<#if table.isExternal==0>id<#else>${table.pkField}</#if>}');" class="link run"><span></span>提交</a>
					</c:if>
					<#else>
					<a href="del.ht?id=<#noparse>${</#noparse>${classVar}Item.<#if table.isExternal==0>id<#else>${table.pkField}</#if>}" class="link del"><span></span>删除</a>
					</#if>
