...
import com.suneee.eas.gateway.filter.AuthenticationFilter;
...
public class SessionRepositoryFilter<S extends Session> extends OncePerRequestFilter {
	 ...
	 * @param request
	 */
	private void processSessionId(HttpServletRequest request,HttpServletResponse response){
		...
		HashMap<String,Object> tokenBody = (HashMap<String, Object>) redisTemplate.opsForValue().get(AuthenticationFilter.PREFIX_AUTHORITY_TOKEN+sessionId);
		if (tokenBody==null ||StringUtil.isEmpty((String) tokenBody.get(MyCookieSerializer.KEY_SESSION_ID))){
			return;
		}
		String authId= (String) tokenBody.get(MyCookieSerializer.KEY_SESSION_ID);
		request.setAttribute(MyCookieSerializer.KEY_SESSION_ID,authId);

		String paramEnterpriseCode=getParamEnterpriseCode(request);
		String cacheEnterpriseCode= (String) tokenBody.get("enterpriseCode");
		if (StringUtil.isNotEmpty(paramEnterpriseCode)&&!cacheEnterpriseCode.equals(paramEnterpriseCode)){
			tokenBody.put("enterpriseCode",paramEnterpriseCode);
			redisTemplate.opsForValue().set(AuthenticationFilter.PREFIX_AUTHORITY_TOKEN+sessionId,tokenBody,1800, TimeUnit.SECONDS);
			Enterpriseinfo enterpriseinfo= null;
			try {
				enterpriseinfo = enterpriseInfoService.getByCompCode(paramEnterpriseCode);
			} catch (UnsupportedEncodingException e) {
				e.printStackTrace();
			}
			if (enterpriseinfo==null){
				SESSION_LOGGER.warn("用户所属企业("+paramEnterpriseCode+")不存在，默认使用当前企业("+cacheEnterpriseCode+")");
				return;
			}
			request.getSession().setAttribute(UserConstant.SESSION_ENTERPRISE_KEY,enterpriseinfo);
	 ...
	 * @return
	 */
	protected String getParamEnterpriseCode(HttpServletRequest request) {
	...
	protected void doFilterInternal(HttpServletRequest request,
		...
		processSessionId(request,response);
	 ...
	 * @param response
	 */
	public void commitSession(HttpServletRequest request,
